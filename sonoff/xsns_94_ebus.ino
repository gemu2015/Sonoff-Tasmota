...
const char JSON_EBUS[] PROGMEM = "%s,\"%s\":{\"" "Outsidetemp" "\":%d,\"" "Roomtemp" "\":%d,\"" "Boiler" "\":%d,\"" "Returns" "\":%d,\"" "Warmwater" "\":%d,\"" "Burner" "\":%d,\"" "Status" "\":%d,\"" "Solarstorage" "\":%d,\"" "Collector" "\":%d,\"" "Solarpump" "\":%d }";

const char WEB_EBUS[] PROGMEM = "%s"
...
void EBUS_Show(boolean json) {
	if (json) {
      snprintf_P(mqtt_data, sizeof(mqtt_data), JSON_EBUS, mqtt_data, "EBUS",alldata.Aussensensor,alldata.Raumtemperatur,alldata.Heizkessel,alldata.Ruecklauf,alldata.Warmwasser,alldata.Brenner,alldata.Status,alldata.Solarspeicher,alldata.Kollektor,alldata.Solarpumpe);
  } else {
#ifdef USE_WEBSERVER
      snprintf_P(mqtt_data, sizeof(mqtt_data), WEB_EBUS, mqtt_data,alldata.Aussensensor,alldata.Raumtemperatur,alldata.Heizkessel,alldata.Ruecklauf,alldata.Warmwasser,alldata.Brenner,alldata.Status,alldata.Solarspeicher,alldata.Kollektor,alldata.Solarpumpe);
#endif
  }
}
