...
static void p_shine_huffman_coder_count1(bitstream_t *bs, const struct huffcodetab *h, int v, int w, int x, int y);

static void encodeSideInfo( p_shine_global_config *config );
static void encodeMainData( p_shine_global_config *config );
static void Huffmancodebits( p_shine_global_config *config, int *ix, gr_info *gi);

/*
  p_shine_format_bitstream()
...
static void encodeMainData(p_shine_global_config *config) {
  int gr, ch, sfb;
  p_shine_side_info_t  si = config->side_info;
      ...
      for ( ch = 0; ch < config->wave.channels; ch++ )
        {
          ...
          unsigned slen1 = p_shine_slen1_tab[ gi->scalefac_compress ];
          unsigned slen2 = p_shine_slen2_tab[ gi->scalefac_compress ];
...
static void Huffmancodebits( p_shine_global_config *config, int *ix, gr_info *gi ) {
  const int *scalefac = &p_shine_scale_fact_band_index[config->mpeg.samplerate_index][0];
...
static inline int p_shine_abs_and_sign( int *x ) {
  if ( *x > 0 ) return 0;
  *x *= -1;
  return 1;
}
...
static void p_shine_huffman_coder_count1( bitstream_t *bs, const struct huffcodetab *h, int v, int w, int x, int y ) {
  ...
  signv = p_shine_abs_and_sign( &v );
  signw = p_shine_abs_and_sign( &w );
  signx = p_shine_abs_and_sign( &x );
  signy = p_shine_abs_and_sign( &y );
