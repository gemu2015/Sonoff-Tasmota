 ...
 *  @param  value
 */
MODULE_PART void Adafruit_TCS34725::write8(uint8_t reg, uint32_t value) {
 ...
 *  @return value
 */
MODULE_PART uint8_t Adafruit_TCS34725::read8(uint8_t reg) {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
  _wire->write(TCS34725_COMMAND_BIT | reg);
#else
 ...
 *  @return value
 */
MODULE_PART uint16_t Adafruit_TCS34725::read16(uint8_t reg) {
  uint16_t x;
  uint16_t t;
 ...
 */
MODULE_PART void Adafruit_TCS34725::enable() {
SETMINREGS
 ...
 */
MODULE_PART void Adafruit_TCS34725::disable() {
 ...
 *          Gain
 */
MODULE_PART Adafruit_TCS34725::Adafruit_TCS34725(tcs34725IntegrationTime_t it,
                                     tcs34725Gain_t gain) {
 ...
 *  @return True if initialization was successful, otherwise false.
 */
MODULE_PART boolean Adafruit_TCS34725::begin(uint8_t addr, TwoWire *theWire) {
  _i2caddr = addr;
  _wire = theWire;
  return init();
}
 ...
 *          Integration Time
 */
MODULE_PART void Adafruit_TCS34725::setIntegrationTime(tcs34725IntegrationTime_t it) {
 ...
 */
MODULE_PART void Adafruit_TCS34725::setGain(tcs34725Gain_t gain) {
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
MODULE_PART void Adafruit_TCS34725::getRawData(uint16_t *r, uint16_t *g, uint16_t *b,
                                   uint16_t *c) {
SETMINREGS
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
MODULE_PART void Adafruit_TCS34725::getRawDataOneShot(uint16_t *r, uint16_t *g, uint16_t *b,
                                          uint16_t *c) {
  if (!_tcs34725Initialised)
    begin();
 ...
 */
MODULE_PART void Adafruit_TCS34725::getRGB(float *r, float *g, float *b) {
 ...
 */
MODULE_PART uint16_t Adafruit_TCS34725::calculateColorTemperature(uint16_t r, uint16_t g,
                                                      uint16_t b) {
 ...
 *  @return Color temperature in degrees Kelvin
 */
MODULE_PART uint16_t Adafruit_TCS34725::calculateColorTemperature_dn40(uint16_t r,
                                                           uint16_t g,
                                                           uint16_t b,
                                                           uint16_t c) {
 ...
 *  @return Lux value
 */
MODULE_PART uint16_t Adafruit_TCS34725::calculateLux(uint16_t r, uint16_t g, uint16_t b) {
  float illuminance;
 ...
 */
MODULE_PART void Adafruit_TCS34725::setInterrupt(boolean i) {
 ...
 */
MODULE_PART void Adafruit_TCS34725::clearInterrupt() {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
 ...
 *          High limit
 */
MODULE_PART void Adafruit_TCS34725::setIntLimits(uint16_t low, uint16_t high) {
