...
static void MODULE_PART read_dn(mtrc_tlv_reader *r, mtrc_cert *c, int is_subject) {
  mtrc_tlv_elem e;
...
static void MODULE_PART read_ext(mtrc_tlv_reader *r, mtrc_cert *c) {
  mtrc_tlv_elem e;
  while (mtrc_tlv_read(r, &e) && e.type != MTRC_TLV_END) {
...
int MODULE_PART mtrc_cert_parse(const uint8_t *tlv, size_t len, mtrc_cert *out) {
  if (!tlv || !out) return 0;
...
int MODULE_PART mtrc_cert_chain_check(const mtrc_cert *noc, const mtrc_cert *icac,
