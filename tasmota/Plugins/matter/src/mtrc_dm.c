...
int MODULE_PART mtrc_dm_add_endpoint(uint16_t endpoint, uint32_t device_type) {
...
int MODULE_PART mtrc_dm_add_cluster(uint16_t endpoint, uint32_t cluster) {
...
int MODULE_PART mtrc_dm_add_attr(uint16_t endpoint, uint32_t cluster, uint32_t attr,
...
mtrc_dm_attr_t *MODULE_PART mtrc_dm_find(uint16_t endpoint, uint32_t cluster, uint32_t attr) {
...
int MODULE_PART mtrc_dm_get(uint16_t endpoint, uint32_t cluster, uint32_t attr, uint64_t *out) {
  mtrc_dm_attr_t *a = mtrc_dm_find(endpoint, cluster, attr);
  if (!a) return 0;
  if (out) *out = a->value;
  return 1;
...
int MODULE_PART mtrc_dm_set(uint16_t endpoint, uint32_t cluster, uint32_t attr, uint64_t value) {
  mtrc_dm_attr_t *a = mtrc_dm_find(endpoint, cluster, attr);
  if (!a) return 0;
...
int MODULE_PART mtrc_dm_endpoint_count(void) { return dm.ep_n; }
const mtrc_dm_endpoint_t *MODULE_PART mtrc_dm_endpoint_at(int i) {
  return (i >= 0 && i < dm.ep_n) ? &dm.ep[i] : NULL;
}
int MODULE_PART mtrc_dm_cluster_count(void) { return dm.cl_n; }
const mtrc_dm_cluster_t *MODULE_PART mtrc_dm_cluster_at(int i) {
  return (i >= 0 && i < dm.cl_n) ? &dm.cl[i] : NULL;
}
int MODULE_PART mtrc_dm_attr_count(void) { return dm.at_n; }
const mtrc_dm_attr_t *MODULE_PART mtrc_dm_attr_at(int i) {
  return (i >= 0 && i < dm.at_n) ? &dm.at[i] : NULL;
}

int MODULE_PART mtrc_dm_endpoint_device_type(uint16_t endpoint, uint32_t *out) {
  for (int i = 0; i < dm.ep_n; i++)
