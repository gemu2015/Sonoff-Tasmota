...
MODULE_PART void wb_u8 (wbuf *w, uint8_t v){ SETMEMREGS if(w->err||w->len+1>w->cap){w->err=1;return;} w->p[w->len++]=v; }
MODULE_PART void wb_le (wbuf *w, uint64_t v, int n){ SETMEMREGS for(int i=0;i<n;i++) wb_u8(w,(uint8_t)(v>>(8*i))); }
MODULE_PART void wb_raw(wbuf *w, const uint8_t *d, size_t n){ SETMEMREGS
  if(w->err||w->len+n>w->cap){w->err=1;return;} if(n){memcpy(w->p+w->len,d,n); w->len+=n;}
}

MODULE_PART uint8_t mtrc_frame_security_flags(const mtrc_msg_header *mh) { SETMEMREGS
  uint8_t sf = (uint8_t)(mh->session_type & SF_STYPE_MASK);
  if (mh->control) sf |= SF_C;
  return sf;
}

MODULE_PART int mtrc_frame_encode_msg_header(uint8_t *out, size_t cap, const mtrc_msg_header *mh) { SETMEMREGS
  if (!out || !mh) return -1;
  wbuf w = { out, cap, 0, 0 };
...
MODULE_PART int rb_avail(rbuf *r, size_t n){ SETMEMREGS return r->off + n <= r->len; }
MODULE_PART uint8_t  rb_u8(rbuf *r){ SETMEMREGS if(!rb_avail(r,1)){r->err=1;return 0;} return r->p[r->off++]; }
MODULE_PART uint64_t rb_le(rbuf *r, int n){ SETMEMREGS
  if(!rb_avail(r,(size_t)n)){r->err=1;return 0;}
  uint64_t v=0; for(int i=0;i<n;i++) v|=(uint64_t)r->p[r->off+i]<<(8*i); r->off+=n; return v;
}

MODULE_PART int mtrc_frame_decode_msg_header(const uint8_t *buf, size_t len, mtrc_msg_header *mh) { SETMEMREGS
  if (!buf || !mh) return -1;
  rbuf r = { buf, len, 0, 0 };
