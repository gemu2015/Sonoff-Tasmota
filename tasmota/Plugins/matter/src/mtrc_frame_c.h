...
uint8_t MODULE_PART mtrc_frame_security_flags(const mtrc_msg_header *mh) {
  uint8_t sf = (uint8_t)(mh->session_type & SF_STYPE_MASK);
  if (mh->control) sf |= SF_C;
  return sf;
}

int MODULE_PART mtrc_frame_encode_msg_header(uint8_t *out, size_t cap, const mtrc_msg_header *mh) {
  if (!out || !mh) return -1;
  wbuf w = { out, cap, 0, 0 };
...
int MODULE_PART mtrc_frame_encode_proto(uint8_t *out, size_t cap, const mtrc_proto_header *ph,
...
int MODULE_PART mtrc_frame_encode(uint8_t *out, size_t cap,
...
static int MODULE_PART rb_avail(rbuf *r, size_t n){ return r->off + n <= r->len; }
...
int MODULE_PART mtrc_frame_decode_proto(const uint8_t *buf, size_t len, mtrc_proto_header *ph,
                            const uint8_t **payload, size_t *payload_len) {
  if (!buf || !ph) return -1;
  rbuf r = { buf, len, 0, 0 };
...
int MODULE_PART mtrc_frame_decode(const uint8_t *buf, size_t len,
