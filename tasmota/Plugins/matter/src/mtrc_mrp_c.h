...
static uint32_t MODULE_PART mrp_interval(uint32_t base, int attempt /*1-based tx count*/) {
...
void MODULE_PART mtrc_mrp_init(mtrc_mrp_ctx *c, uint32_t base_ms) {
  memset(c, 0, sizeof(*c));
  c->base_ms = base_ms ? base_ms : MTRC_MRP_DEFAULT_BASE_MS;
}

int MODULE_PART mtrc_mrp_on_send(mtrc_mrp_ctx *c, const uint8_t *frame, size_t len,
...
int MODULE_PART mtrc_mrp_on_ack(mtrc_mrp_ctx *c, uint32_t ack_counter) {
...
int MODULE_PART mtrc_mrp_is_duplicate(mtrc_mrp_ctx *c, uint32_t msg_counter) {
  if (!c->have_rx) {
...
mtrc_mrp_tick_result MODULE_PART mtrc_mrp_tick(mtrc_mrp_ctx *c, uint32_t now_ms,
...
int MODULE_PART mtrc_mrp_build_ack(uint8_t *out, size_t cap,
