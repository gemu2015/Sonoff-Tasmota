...
MODULE_PART int mtrc_pase_derive_w0w1(uint32_t passcode, const uint8_t *salt, size_t salt_len,
                          uint32_t iterations, uint8_t w0[32], uint8_t w1[32]) { SETMEMREGS
...
MODULE_PART void mtrc_pase_context(const uint8_t *req, size_t req_len,
                       const uint8_t *resp, size_t resp_len, uint8_t ctx[32]) { SETMEMREGS
...
MODULE_PART int mtrc_pase_keys(const uint8_t ctx[32], const uint8_t pA[65], const uint8_t pB[65],
                   ...
                   mtrc_pase_keys_t *out) { SETMEMREGS
  uint8_t K_main[32];
...
MODULE_PART int mtrc_pase_encode_param_req(uint8_t *out, size_t cap, const mtrc_pase_param_req *r) { SETMEMREGS
...
MODULE_PART int mtrc_pase_decode_param_req(const uint8_t *in, size_t len, mtrc_pase_param_req *r) { SETMEMREGS
...
MODULE_PART int mtrc_pase_encode_param_resp(uint8_t *out, size_t cap, const mtrc_pase_param_resp *r) { SETMEMREGS
  mtrc_tlv_writer w; mtrc_tlv_writer_init(&w, out, cap);
  mtrc_tlv_start_struct(&w, mtrc_tlv_anon());
  mtrc_tlv_put_bytes(&w, mtrc_tlv_ctx(1), r->initiator_random, 32);
...
MODULE_PART int mtrc_pase_decode_param_resp(const uint8_t *in, size_t len, mtrc_pase_param_resp *r) { SETMEMREGS
  mtrc_tlv_reader rd; mtrc_tlv_reader_init(&rd, in, len);
  mtrc_tlv_elem e;
  memset(r, 0, sizeof(*r));
  if (!mtrc_tlv_read(&rd, &e) || e.type != MTRC_TLV_STRUCT) return 0;
...
MODULE_PART int enc_one_bytes(uint8_t *out, size_t cap, uint8_t tag, const uint8_t *d, size_t n) { SETMEMREGS
  mtrc_tlv_writer w; mtrc_tlv_writer_init(&w, out, cap);
  mtrc_tlv_start_struct(&w, mtrc_tlv_anon());
...
MODULE_PART int dec_field_bytes(const uint8_t *in, size_t len, uint8_t tag, uint8_t *d, size_t n) { SETMEMREGS
  mtrc_tlv_reader rd; mtrc_tlv_reader_init(&rd, in, len);
  mtrc_tlv_elem e;
  if (!mtrc_tlv_read(&rd, &e) || e.type != MTRC_TLV_STRUCT) return 0;
  int found = 0;
...
MODULE_PART int mtrc_pase_encode_pake1(uint8_t *out, size_t cap, const uint8_t pA[65]) { SETMEMREGS return enc_one_bytes(out, cap, 1, pA, 65); }
MODULE_PART int mtrc_pase_decode_pake1(const uint8_t *in, size_t len, uint8_t pA[65])  { SETMEMREGS return dec_field_bytes(in, len, 1, pA, 65); }
MODULE_PART int mtrc_pase_encode_pake3(uint8_t *out, size_t cap, const uint8_t cA[32]) { SETMEMREGS return enc_one_bytes(out, cap, 1, cA, 32); }
MODULE_PART int mtrc_pase_decode_pake3(const uint8_t *in, size_t len, uint8_t cA[32])  { SETMEMREGS return dec_field_bytes(in, len, 1, cA, 32); }

MODULE_PART int mtrc_pase_encode_pake2(uint8_t *out, size_t cap, const uint8_t pB[65], const uint8_t cB[32]) { SETMEMREGS
  mtrc_tlv_writer w; mtrc_tlv_writer_init(&w, out, cap);
  mtrc_tlv_start_struct(&w, mtrc_tlv_anon());
...
MODULE_PART int mtrc_pase_decode_pake2(const uint8_t *in, size_t len, uint8_t pB[65], uint8_t cB[32]) { SETMEMREGS
  mtrc_tlv_reader rd; mtrc_tlv_reader_init(&rd, in, len);
