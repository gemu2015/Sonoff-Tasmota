...
const uint8_t SPAKE_M[65] PROGMEM = {
  0x04,
...
const uint8_t SPAKE_N[65] PROGMEM = {
  0x04,
...
MODULE_PART int mtrc_spake2p_prover_ZV(const uint8_t w0[32], const uint8_t w1[32],
  ...
  memcpy(T, Y, 65);
  if (!mtrc_ec_muladd(T, MP8(SPAKE_N), MP8(SCALAR_ONE), 1, neg_w0, 32)) return 0;
  // Z = x*T,  V = w1*T
  memcpy(Z, T, 65);
...
MODULE_PART int mtrc_spake2p_verifier_ZV(const uint8_t w0[32], const uint8_t y[32],
  ...
  mtrc_ec_scalar_neg(w0, neg_w0);
  memcpy(T, X, 65);
  if (!mtrc_ec_muladd(T, MP8(SPAKE_M), MP8(SCALAR_ONE), 1, neg_w0, 32)) return 0;
  // Z = y*T,  V = y*L
  memcpy(Z, T, 65);
...
MODULE_PART int mtrc_spake2p_transcript(const uint8_t *context, size_t context_len,
  ...
  tt_put(tt, &off, MP8(SPAKE_M), 65);
  tt_put(tt, &off, MP8(SPAKE_N), 65);
  tt_put(tt, &off, X, 65);
  tt_put(tt, &off, Y, 65);
  tt_put(tt, &off, Z, 65);
  tt_put(tt, &off, V, 65);
