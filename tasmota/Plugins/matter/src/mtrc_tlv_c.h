...
// ---- writer low-level --------------------------------------------------
static int MODULE_PART w_raw(mtrc_tlv_writer *w, const uint8_t *p, size_t n) {
  if (w->err) return 0;
...
static int MODULE_PART w_byte(mtrc_tlv_writer *w, uint8_t b) { return w_raw(w, &b, 1); }
...
static int MODULE_PART w_tag(mtrc_tlv_writer *w, mtrc_tlv_tag tag) {
  switch (tag.ctrl) {
...
static int MODULE_PART w_head(mtrc_tlv_writer *w, mtrc_tlv_tag tag, uint8_t elem_type) {
  if (!w_byte(w, (uint8_t)(((uint8_t)tag.ctrl << 5) | elem_type))) return 0;
  return w_tag(w, tag);
}

void MODULE_PART mtrc_tlv_writer_init(mtrc_tlv_writer *w, uint8_t *buf, size_t cap) {
  w->buf = buf; w->cap = cap; w->len = 0; w->depth = 0; w->err = 0;
}
size_t MODULE_PART mtrc_tlv_writer_len(const mtrc_tlv_writer *w) { return w->len; }
int    MODULE_PART mtrc_tlv_writer_ok (const mtrc_tlv_writer *w) { return !w->err && w->depth == 0; }
...
int MODULE_PART mtrc_tlv_put_raw(mtrc_tlv_writer *w, const uint8_t *p, size_t n) { return w_raw(w, p, n); }

int MODULE_PART mtrc_tlv_put_uint(mtrc_tlv_writer *w, mtrc_tlv_tag tag, uint64_t v) {
  uint8_t t; int n;
...
int MODULE_PART mtrc_tlv_put_int(mtrc_tlv_writer *w, mtrc_tlv_tag tag, int64_t v) {
  uint8_t t; int n;
...
int MODULE_PART mtrc_tlv_put_bool(mtrc_tlv_writer *w, mtrc_tlv_tag tag, bool b) {
  return w_head(w, tag, (uint8_t)(MTRC_TLV_BOOL + (b ? 1 : 0)));
}
int MODULE_PART mtrc_tlv_put_null(mtrc_tlv_writer *w, mtrc_tlv_tag tag) {
  return w_head(w, tag, MTRC_TLV_NULL);
}
int MODULE_PART mtrc_tlv_put_float(mtrc_tlv_writer *w, mtrc_tlv_tag tag, float f) {
  uint32_t u; memcpy(&u, &f, 4);
  return w_head(w, tag, MTRC_TLV_FLOAT) && w_le(w, u, 4);
}
int MODULE_PART mtrc_tlv_put_double(mtrc_tlv_writer *w, mtrc_tlv_tag tag, double dd) {
...
static int MODULE_PART w_lenval(mtrc_tlv_writer *w, mtrc_tlv_tag tag, uint8_t base,
...
int MODULE_PART mtrc_tlv_put_bytes(mtrc_tlv_writer *w, mtrc_tlv_tag tag,
                       const uint8_t *data, size_t len) {
  return w_lenval(w, tag, MTRC_TLV_BYTES, data, len);
}
int MODULE_PART mtrc_tlv_put_utf8(mtrc_tlv_writer *w, mtrc_tlv_tag tag,
                      const char *s, size_t len) {
  return w_lenval(w, tag, MTRC_TLV_UTF8, (const uint8_t *)s, len);
}

static int MODULE_PART w_start(mtrc_tlv_writer *w, mtrc_tlv_tag tag, uint8_t t) {
  if (!w_head(w, tag, t)) return 0;
  w->depth++;
  return 1;
}
int MODULE_PART mtrc_tlv_start_struct(mtrc_tlv_writer *w, mtrc_tlv_tag tag) { return w_start(w, tag, MTRC_TLV_STRUCT); }
int MODULE_PART mtrc_tlv_start_array (mtrc_tlv_writer *w, mtrc_tlv_tag tag) { return w_start(w, tag, MTRC_TLV_ARRAY); }
int MODULE_PART mtrc_tlv_start_list  (mtrc_tlv_writer *w, mtrc_tlv_tag tag) { return w_start(w, tag, MTRC_TLV_LIST); }
int MODULE_PART mtrc_tlv_end_container(mtrc_tlv_writer *w) {
...
static int MODULE_PART r_avail(mtrc_tlv_reader *r, size_t n) { return r->off <= r->len && n <= r->len - r->off; }
...
void MODULE_PART mtrc_tlv_reader_init(mtrc_tlv_reader *r, const uint8_t *buf, size_t len) {
  r->buf = buf; r->len = len; r->off = 0; r->err = 0;
}

int MODULE_PART mtrc_tlv_read(mtrc_tlv_reader *r, mtrc_tlv_elem *e) {
