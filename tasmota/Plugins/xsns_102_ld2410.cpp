...
#define LD2410_BUFFER_SIZE               64  // TM_SERIAL_BUFFER_SIZE  // 64
...
/********************************************************************************************/
PUSH_OPTIONS
MODULE_DESCRIPTOR("LD2410",MODULE_TYPE_SENSOR,1<<16|4,"RXD",5,"TXD",6,"",0,"",0)
...
MODULE_PART void Ld2410SendCommand(uint32_t command, uint8_t *val, uint32_t val_len);
MODULE_PART void Ld2410SetConfigMode(void);
MODULE_PART void Ld2410SetMaxDistancesAndNoneDuration(uint32_t max_moving_distance_range, uint32_t max_static_distance_range, uint32_t no_one_duration);
...
*/
  ...
  while (availTS(LD2410.ts)) {
    ...
    LD2410.buffer[LD2410.byte_counter++] = jread1TS(LD2410.ts);
...
//        AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("LD2: Rcvd %*_H"), len, LD2410.buffer);
        ...
        if (Ld2410Match(LD2410.config_footer, len -4)) {        // 04030201
          Ld1410HandleConfigData();
          //LD2410.ts->setReadChunkMode(0);                    // Disable chunk mode fixing Hardware Watchdogs
        }
      }
    }
...
void Ld2410SendCommand(uint32_t command, uint8_t *val, uint32_t val_len) {
  ...
  //LD2410.ts->setReadChunkMode(1);                            // Enable chunk mode introducing possible Hardware Watchdogs
  //LD2410.ts->flush();
  flushTS(LD2410.ts);
  //LD2410.ts->write(buffer, len);
...
const char started[] PROGMEM = "LD2410 inizialized with REC pin %d - TRX pin %d";

int32_t Ld2410Detect(void) {
  ALLOCMEM

  rxd_pin = mp->ms[0].value;
  txd_pin = mp->ms[1].value;

  ready = false;
  
  LD2410.ts = NewTS(rxd_pin, txd_pin);
  
  if (LD2410.ts) {
    if (beginTS(LD2410.ts, ICONST(256000))) {
      if (hardwareSerial(LD2410.ts)) {
        ClaimSerial();
      }
      AddLog(LOG_LEVEL_INFO, GSTR(started), rxd_pin, txd_pin);
...
/*
      LD2410.config_header[0] = 0xFD;
      LD2410.config_header[1] = 0xFC;
      LD2410.config_header[2] = 0xFB;
      LD2410.config_header[3] = 0xFA;

      LD2410.config_footer[0] = 0x04;
      LD2410.config_footer[1] = 0x03;
      LD2410.config_footer[2] = 0x02;
      LD2410.config_footer[3] = 0x01;

      LD2410.target_header[0] = 0xF4;
      LD2410.target_header[1] = 0xF3;
      LD2410.target_header[2] = 0xF2;
      LD2410.target_header[3] = 0xF1;

      LD2410.target_footer[0] = 0xF8;
      LD2410.target_footer[1] = 0xF7;
      LD2410.target_footer[2] = 0xF6;
      LD2410.target_footer[3] = 0xF5;
      */
...
void CmndLd2410MovingSensitivity(void) {
 ...
 // uint32_t parm[LD2410_MAX_GATES +1];
...
void LD2410_Deinit(void) {
  SETREGS
  if (LD2410.ts) deleteTS(LD2410.ts);
  RETMEM
}
/*********************************************************************************************\
 * Interface
