/*
  xsns_125_bresser_dual.cpp — Bresser Weather Sensor CC1101 driver,
  dual-format (BinPlugin + native firmware).

  Original copyright preserved:
    Copyright (C) 2024 Gerhard Mutz, Matthias Prinke
                       (BresserWeatherSensorReceiver)

  Plugin: USE_BRESSER_DUAL_MOD via build_plugin.py.
  Native: USE_BRESSER_DUAL via the shim at
          tasmota/tasmota_xsns_sensor/xsns_125_bresser_dual.ino.

  Largest dual conversion (~1300 lines) and the second to use SPI
  (after PN532's I2C+HSU). Drives a CC1101 RF transceiver at
  868.3 MHz to receive Bresser weather-station broadcasts —
  decoders for 5-in-1, 6-in-1, 7-in-1, lightning, leakage, soil,
  air-quality (PM/CO2/HCHO-VOC) protocols.

  Body uses the LTR308-style in-function `MODULE_MEMORY *mem`
  pattern (via SETMEMREGS) — function bodies use `mem->field`
  access bit-identically in both modes. SPI helpers
  (spiBeginTransaction / spiTransfer / spi_begin / etc.) get a
  native shim below that routes to Arduino's SPI library directly
  using mem->spibaud — call sites unchanged.
*/

#include "tasmota_options.h"

#ifndef BUILD_AS_PLUGIN
#  ifdef USE_BRESSER_DUAL_MOD
...
#if BUILD_AS_PLUGIN
#  ifdef USE_BRESSER_DUAL_MOD
#    define _BRESSER_DUAL_ENABLED 1
#  endif
#else
#  if defined(USE_BRESSER_DUAL) && defined(BRESSER_DUAL_NATIVE_INCLUDE)
#    define _BRESSER_DUAL_ENABLED 1
#  endif
#endif

#ifdef _BRESSER_DUAL_ENABLED
...
/*********************************************************************************************/
// State storage — heap in both modes. Plugin keeps the legacy
// MODULE_MEMORY name; native uses a file-unique typedef
...
#  define spiBeginTransaction()  SPI.beginTransaction(SPISettings(mem->spibaud, MSBFIRST, SPI_MODE0))
#  define spiEndTransaction()    SPI.endTransaction()
#  define spiTransfer(b)         SPI.transfer((uint8_t)(b))
#  define spi_begin()            SPI.begin()
#  define spi_end()              SPI.end()
#  define getspi(bus)            ((void *)&SPI)
#  define GETSPI(bus)            do { mem->spi = (void *)&SPI; } while (0)

#  define XSNS_125               125

#endif  // BUILD_AS_PLUGIN

#define BRESSER_REV (1 << 16 | 5)

// XdrvMailbox accessor (pointer in plugin, instance in native)
#if BUILD_AS_PLUGIN
#  define _BRES_MB_DATA      (XdrvMailbox->data)
#  define _BRES_MB_DATA_LEN  (XdrvMailbox->data_len)
#  define _BRES_TG_TELE      (TasmotaGlobal->tele_period)
#else
#  define _BRES_MB_DATA      (XdrvMailbox.data)
#  define _BRES_MB_DATA_LEN  (XdrvMailbox.data_len)
#  define _BRES_TG_TELE      (TasmotaGlobal.tele_period)
...
// --------------------------------------------------------------------
// Plugin descriptor block (canonical layout — all decls between
// MODULE_DESCRIPTOR and MODULE_END so they land in SECTION_PART)
// --------------------------------------------------------------------
PUSH_OPTIONS
MODULE_DESCRIPTOR("BRESSER", MODULE_TYPE_SENSOR, BRESSER_REV,
                  "CS", 15, "GDO", 5, "", 0, "", 0)
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
      ...
      if (mem->sensor_copy[i].s_type == SENSOR_TYPE_SOIL) {
        ...
        if (mem->sensor_copy[i].w.temp_ok && mem->sensor_copy[i].w.humidity_ok) {
          TempHumDewShow(json, (0 == _BRES_TG_TELE), label, mem->sensor_copy[i].w.temp_c, mem->sensor_copy[i].w.humidity);
        }
      } else {
        if (mem->sensor_copy[i].w.temp_ok && mem->sensor_copy[i].w.humidity_ok) {
          TempHumDewShow(json, (0 == _BRES_TG_TELE), label, mem->sensor_copy[i].w.temp_c, mem->sensor_copy[i].w.humidity);
...
MODULE_PART bool Bresser_Cmd() {
  SETMEMREGS
  char *cp = _BRES_MB_DATA;
  if (_BRES_MB_DATA_LEN > 0) {
    if (*cp == 'r') {
      cp++;
      uint8_t index = *cp & 7;
...
MOD_RESULT mod_func_execute(uint32_t sel) {
  ...
  switch (sel) {
    case pFUNC_INIT:              result = Bresser_Init();   break;
    case pFUNC_EVERY_100_MSECOND: Bresser_Task();            break;
    case pFUNC_JSON_APPEND:       Bresser_Show(1);           break;
    case pFUNC_WEB_SENSOR:        Bresser_Show(0);           break;
    case pFUNC_COMMAND_SENSOR:    result = Bresser_Cmd();    break;
    case pFUNC_DEINIT:            Bresser_Deinit();          break;
  }
  return result;
}
...
bool Xsns125(uint32_t function) {
  ...
  if (FUNC_INIT == function) {
    Bresser_Init();
  } else if (bresser_state && initialized) {
    switch (function) {
      case FUNC_EVERY_100_MSECOND: Bresser_Task();         break;
      case FUNC_JSON_APPEND:       Bresser_Show(1);        break;
#  ifdef USE_WEBSERVER
      case FUNC_WEB_SENSOR:        Bresser_Show(0);        break;
#  endif
      case FUNC_COMMAND_SENSOR:    result = Bresser_Cmd(); break;
    }
  }
  return result;
}
...
#endif
#undef _BRES_MB_DATA
#undef _BRES_MB_DATA_LEN
#undef _BRES_TG_TELE

#endif  // _BRESSER_DUAL_ENABLED
