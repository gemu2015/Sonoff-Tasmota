...
MODULE_PART void ccTX(void);
MODULE_PART void ccRX(void);
MODULE_PART uint8_t ccStrobe(uint8_t);
MODULE_PART void ccreg(char *);
MODULE_PART void ccsetpa(char *);
MODULE_PART uint8_t cc1100_sendbyte(uint8_t data);
MODULE_PART void cc1100_writeReg(uint8_t addr, uint8_t data);
...
MODULE_PART void moritz_sendraw(uint8_t *buf, int longPreamble);
MODULE_PART void moritz_sendAck(uint8_t *enc);
MODULE_PART void moritz_handleAutoAck(uint8_t *enc);
MODULE_PART void display_char(char c);
MODULE_PART void display_string(char *s);
MODULE_PART void display_string_P(const char *s);
MODULE_PART void display_nl(void);
MODULE_PART void display_hex(uint16_t h, int8_t pad, uint8_t padc);
MODULE_PART void display_hex2(uint8_t h);
MODULE_PART void ccRX(void);
MODULE_PART void ccTX(void);
MODULE_PART void set_ccoff(void);
MODULE_PART void set_ccon(void);
MODULE_PART int fromhex(const char *in, uint8_t *out, uint8_t buflen);
MODULE_PART void checkFrequency(void);
...
MODULE_PART bool Moritz_Detect();
...
const char EEP_FNAME[] PROGMEM = "/ffs/moritz_data.bin";

void eeprom_writeBytes(uint32_t offset, uint32_t size, void *buff) {
  SETREGS
  File_p *fp;
  fp = fopen(GSTR(EEP_FNAME), 'U');
  if (fp) {
...
void eeprom_readBytes(uint32_t offset, uint32_t size, void *buff) {
  ...
  if (fp) {
    fseek(fp, offset, SEEK_SET);
    uint16_t len = fread(buff, 1, size, fp);
    fclose(fp);
  }
}
...
typedef enum { CC_Pin_Out = 0, CC_Pin_CS, CC_Pin_In } CC_PIN;
...
const uint8_t PROGMEM MORITZ_CFG_TABLE[60] = {
    //     0x00, 0x08,
    0x00, 0x07,  // IOCFG2: GDO2_CFG=7: Asserts when a packet has been received with CRC OK. De-asserts when the first byte is read from the
                 // RX FIFO
    // 0x02, 0x46, //IOCFG0
    ...
    0x0F, 0x6A,  // FREQ0
    0x07, 0x0C,  // PKTCTRL1
    0x16, 0x07,  // MCSM2 RX_TIME = 7 (Timeout for sync word search in RX for both WOR mode and normal RX operation = Until end of packet)
                 // RX_TIME_QUAL=0 (check if sync word is found)
...
void ccRX(void) {
  SETREGS
  uint8_t cnt = 0xff;
  while (cnt-- && (ccStrobe(CC1100_SRX) & CC1100_STATUS_STATE_BM) != CC1100_STATE_RX) delayMicroseconds(10);
}
...
void ccTX(void) {
  ...
  while (cnt-- && (ccStrobe(CC1100_STX) & 0x70) != 2) delayMicroseconds(10);
}
...
SETREGS
  ...
  CC1100_ASSERT;
  delayMicroseconds(100);  // 30
  CC1100_DEASSERT;
  delayMicroseconds(200);  // 45
...
void rf_moritz_init(void) {
  ...
  // load configuration
  uint8_t *cfgbuf = (uint8_t*)special_malloc(sizeof(MORITZ_CFG_TABLE));
  if (cfgbuf) {
    memmove_P(cfgbuf, GU8(MORITZ_CFG_TABLE), sizeof(MORITZ_CFG_TABLE));
    for (uint8_t i = 0; i < 60; i += 2) {
      if (cfgbuf[i] > 0x40) break;
      cc1100_writeReg(cfgbuf[i], cfgbuf[i + 1]);
    }
    free(cfgbuf);
  }
  ...
  while (cnt-- && (ccStrobe(CC1100_SRX) & 0x70) != 1) delayMicroseconds(10);
...
void moritz_sendraw(uint8_t *dec, int longPreamble) {
  ...
  if (longPreamble) {
    ...
    for (int i = 0; i < 10; ++i) delay(100);  // arg is uint_8, so loop
  }
 ...
 // }
  ...
  for (i = 0; i < 250; ++i) {
    ...
    if (stat1 == MARCSTATE_RX) break;  // now in RX, good
...
const char HTTP_MORITZ_CSS[] PROGMEM =
    ...
    "<tr align='center';><th>TYPE</th><th align='left'>Moritz "
    "ID</th><th>RFE</th><th>BLOW</th><th>RSSI</th><th>MODE</th><th>enabled<br>target temp</th><th>state<br>actual temp</th></tr>";
const char HTTP_MORITZ_TEND[] PROGMEM = "</table>{m}{e}";

const char HTTP_MORITZ_COMMON[] PROGMEM =
    "<tr align='center' style ='background-color: #%s'>"
    "<td><yc>%s</yc></td> <td align='left'><input type='text' maxlength='%d' value='%s' onfocusin='pr(0)' onfocusout='pr(1)' "
    "onchange='miva(value,\"%s\")'></td> <td>%s</td><td>%s</td><td>%d dB</td>";

//"<div><center><label><b>%s</b><input type='text'  value='%s' style='width:200px'  onfocusin='pr(0)' onfocusout='pr(1)'
//onchange='siva(value,\"%s\")'></label></div>";

const char HTTP_MORITZ_PBUT[] PROGMEM = "<td></td><td></td><td>%s</td></tr>";

const char HTTP_MORITZ_WC[] PROGMEM = "<td></td><td><input type='checkbox' %s onchange='miva(%d,\"%s\")';></td><td>%s</td></tr>";

const char HTTP_MORITZ_THERM[] PROGMEM =
    "<td>%s</td><td><input   style='color:black;' min='10' max='30' step='0.5' value='%s' type='number'  onfocusin='pr(0)' "
    "onfocusout='pr(1)' onchange='miva(value,\"%s\")'>"
    "</td><td>%s ºC</td></tr>";
...
SETREGS
  ...
  if (WebServer_hasArg(PSTR("moritz")) ) {
    ...
    } else if (!strncmp_P(cp, PSTR("tmp"), 3)) {
      // set temperature
      cp += 3;
      ind = strtol(cp, &cp, 10);
      cp++;
...
SETREGS
  ...
  for (uint16_t cnt = 0; cnt < MORITZ_MAX_DEVICES; cnt++) {
    memmove_P(&mo, &moritz_devices[cnt] , sizeof(MORITZ_MEM));
...
ALLOCMEM
  ...
  if (TasmotaGlobal->spi_enabled) {
    ...
    GETSPI(0);
    this->spibaud = ICONST(500000);
 
    credit_10ms = ICONST(MAX_CREDIT/2);
    ...
    if (!fexists(GSTR(EEP_FNAME))) {
      ...
      uint8_t *buf = (uint8_t *)calloc(MORITZ_FSIZE, 1);
      if (buf) {
        File_p *fp;
        fp = fopen(GSTR(EEP_FNAME), 'w');
        if (fp) {
          fwrite(buf, 1, MORITZ_FSIZE, fp);
          fclose(fp);
        }
        free(buf);
      }
...
SETREGS
  ...
  //memmove_P(&moritz_devices[index] , ml, sizeof(MORITZ_MEM));
  memmove_P(ml, &moritz_devices[index] , sizeof(MORITZ_MEM));
}
...
SETREGS
  ...
  if (moritz_cfg.moritz_ready) {
    if (millis() - last_moritz_task > 8) {
      rf_moritz_task();
      last_moritz_task = millis();
      if (credit_10ms < ICONST(MAX_CREDIT)) {
...
MODULE_PART void Moritz_List();
MODULE_PART void Moritz_Reset();
