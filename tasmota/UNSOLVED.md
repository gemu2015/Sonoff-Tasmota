...
Every time `arendst/Tasmota` ships meaningful upstream changes, this
fork (`gemu2015/Sonoff-Tasmota`, branch `universal`) needs to absorb
them. Doing this manually is painful because:

- The fork touches many of the same files upstream churns through
  most often (driver registration tables, `tasmota_xdrv_driver/`,
  `include/tasmota_*`, partition tables, `platformio_override.ini`,
  display drivers).
- Many "conflicts" are mechanical and harmless (both sides added a
  new entry to a registration table at neighbouring lines) but git
  flags them anyway and the user has to eyeball-resolve each one.
- Some are genuine semantic conflicts (e.g. upstream renamed a
  function this fork already uses by old name) and need real
  reasoning per hunk.
- After an hour of resolving, fatigue makes mistakes likely; a
  bad merge can break compilation or — worse — silently break
  runtime behaviour in some target build.

### What's been tried

- **Pure manual via `git mergetool` / IDE**: works but slow + error-
  prone. The current state of affairs.
- _(Other approaches not attempted to my knowledge — fill in if there
  were any.)_

### Desired solution sketch

A purpose-built CLI tool that knows enough about this fork's
structure to automate the boring parts and AI-assist the rest:

```
tasmota-merge upstream/development        # fetch + analyse + propose
tasmota-merge --resume                    # continue interactively after a pause
tasmota-merge --finalize                  # apply approved resolutions, single commit
```

#### Internal architecture (sketch)

1. **Fork manifest** — a versioned config file (e.g.
   `.tasmota-fork.yaml`) listing per-file or per-glob policies:
   ```yaml
   merge_policies:
     # Files this fork OWNS — never auto-merge from upstream
     own:
     ...
     # Pristine — always take upstream
     upstream:
       - tasmota/tasmota_xsns_sensor/**
       - lib/libesp32/**

     # Mixed — needs interactive resolution
     mixed:
       - tasmota/tasmota.ino
       - tasmota/include/tasmota.h
       - platformio_override.ini

     # Auto-mergeable structural patterns
     auto_resolve:
       - kind: driver_registration_table
         pattern: "// Driver list:.*\\n.*XDRV_\\d+"
   ```

2. **Three-way diff bucket** — for each file changed in upstream:
   - If `own`: skip (keep fork version).
   - If `upstream`: take upstream verbatim.
   - If `mixed` and there's no textual conflict: 3-way auto-merge.
   - If `mixed` and there IS a conflict: pass each conflict hunk
     (with surrounding context, the file's role in the codebase,
     and recent commit messages from both sides) to Claude API for
     a proposed resolution. Show user `[a]ccept / [e]dit / [s]kip`.
     Cache decisions in a `.tasmota-merge-state.json` for resume.
   - If `auto_resolve` matches: apply the structural rule
     (e.g., merge driver-list additions by index instead of by line
     number).

3. **Decision log** — every resolution gets recorded with:
   timestamp, file, hunk hash, resolution choice (auto / AI / manual
   edit / skip), and a short reason. Becomes a per-merge audit
   trail. Optional: feed back into AI as few-shot examples for
   future merges.

4. **Verification step** — at the end, run a quick sanity pass:
   - `git status` clean? Compile-test on at least one target env
     (e.g. `pio run -e tasmota32-4M`)?
   - Any files in `own` whose content drifted? (Should be zero —
     indicates a bad rule.)

5. **Output** — a single squashed merge commit on the fork's branch
   with a structured commit message listing per-file decisions,
   plus the decision log committed alongside.

#### Why an LLM in the loop helps here specifically

Tasmota merges have lots of "this looks like a conflict but is
trivially mergeable if you know what the file does" cases — the
kind of thing an LLM with the file context can resolve correctly
99 % of the time, and a 1 % miss costs only the 30 seconds it
takes the user to override. Compared to manual eyeballing every
hunk, expected speedup is 5–10×.

#### Estimated effort

- **MVP** (manifest + `own`/`upstream`/`mixed` buckets + interactive
  AI resolution + state cache): ~1 weekend of focused work, Python +
  `gitpython` + Anthropic SDK. ~600–1000 lines.
- **Polish** (auto_resolve rules for driver tables, partition CSVs,
  PIO env entries; decision-log learning; multi-target compile
  verification): another weekend.
- **Stretch** (visual TUI/web UI for the interactive parts; team
  mode where multiple maintainers approve in parallel): a few days
  more, only if shared with other Tasmota fork maintainers.

### Files / conventions worth knowing

| Item | Note |
|---|---|
| Upstream remote | `arendst/Tasmota` (typically) |
| This fork's branch | `gemu2015/Sonoff-Tasmota:universal` |
| Currently divergent areas | TinyC stack, custom xdrv plugins, custom partition layouts, `platformio_override.ini` |
| Custom drivers in fork | xdrv_124 (TinyC), xdrv_14 MP3, custom epaper bits |

### Open questions

- [ ] Is this a **build** project (write the tool) or **outsource**
      project (find an existing fork-maintenance tool)? Quick search
      first — `git-imerge`, `magit-merge-resolve`, `mergiraf`,
      `meld --auto`, etc. exist but none are Tasmota-specific.
- [ ] If building: standalone Python CLI is fine, or worth wrapping
      it as a Claude Code plugin / MCP server so resolution happens
      inside an editor session?
- [ ] How often do upstream merges happen? Daily / weekly / monthly?
      Affects whether MVP needs decision-log learning from day one
      or that's a phase-2 polish.
- [ ] Is there interest from other Tasmota fork maintainers (Staars,
      blakadder, etc.) in collaborating on this? Even one other
      user would justify packaging it as a reusable tool.
