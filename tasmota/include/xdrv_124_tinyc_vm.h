...
#endif
#include "driver/uart.h"   // DMX512 TX: inverse-line BREAK (uart_set_line_inverse)
#endif
...
enum TcSyscall {
  ...
  SYS_DMX_INIT              = 397, // (uart, rx, tx) -> int  1=ok 0=fail
                                  //   Runtime-configure the DMX512 TX UART (uart 0..2,
                                  //   rx -1 = TX-only). Call once before dmxWrite; or set
                                  //   compile-time TC_DMX_TX_PIN for auto-init.
...
// ---------------------------------------------------------------------------
// DMX512 TX engine (TX-only, ESP32). Dedicated hardware UART @250000 8N2.
// Per-frame BREAK via the inverse-line method (uart_set_line_inverse) —
// no library, no per-frame begin(), compiles against pinned arduino-esp32.
//
// Runtime config (no recompile per board):
//   dmxInit(uart, rx, tx)  -> 1 ok / 0 fail   — pick UART + pins at runtime
//   dmxWrite(channel, val)                    — set a slot (1..TC_DMX_SLOTS)
// tc_dmx_tick() (FUNC_EVERY_50_MSECOND, ~20 Hz) emits the frame and runs a
// 30 s watchdog → all-zero (heater safe-off). If dmxInit() is never called
// but the compile-time TC_DMX_TX_PIN is set, it auto-inits from the defines
// (back-compat). DE pin still via #define TC_DMX_DE_PIN (-1 = none/tied).
// ---------------------------------------------------------------------------
#ifdef ESP32
#ifndef TC_DMX_TX_PIN
#define TC_DMX_TX_PIN     -1      // compile-time fallback (dmxInit overrides)
#endif
#ifndef TC_DMX_RX_PIN
#define TC_DMX_RX_PIN     -1
#endif
#ifndef TC_DMX_DE_PIN
#define TC_DMX_DE_PIN     -1
#endif
#ifndef TC_DMX_UART_NUM
#define TC_DMX_UART_NUM   2
#endif
#ifndef TC_DMX_SLOTS
...
static HardwareSerial *tc_dmx_uart = nullptr;
static uint8_t  tc_dmx_buf[TC_DMX_SLOTS];     // slot data (channel 1 = [0])
static bool     tc_dmx_active = false;
static int      tc_dmx_cfg_uart = -1;         // runtime-selected UART num
static uint32_t tc_dmx_last_set_ms = 0;
static bool     tc_dmx_warned = false;

// Runtime (re)configure. uart 0..2, rx/tx GPIO (rx -1 = TX-only).
// Returns 1 on success, 0 on bad args / failure.
static int tc_dmx_init(int uart, int rx, int tx) {
  if (uart < 0 || uart > 2 || tx < 0) {
    AddLog(LOG_LEVEL_INFO,
           PSTR("TCC: dmxInit bad args (uart=%d rx=%d tx=%d)"),
           uart, rx, tx);
    return 0;
  }
  if (tc_dmx_uart) {                           // reconfigure: tear down
    tc_dmx_uart->end();
    delete tc_dmx_uart;
    tc_dmx_uart = nullptr;
  }
  tc_dmx_uart = new HardwareSerial(uart);
  tc_dmx_uart->begin(250000, SERIAL_8N2, rx, tx);  // 250k 8N2
  if (TC_DMX_DE_PIN >= 0) {
    pinMode(TC_DMX_DE_PIN, OUTPUT);
    digitalWrite(TC_DMX_DE_PIN, HIGH);         // RS485 driver enabled (TX)
  }
  memset(tc_dmx_buf, 0, sizeof(tc_dmx_buf));
  tc_dmx_cfg_uart = uart;
  tc_dmx_active = true;
  tc_dmx_last_set_ms = millis();
  AddLog(LOG_LEVEL_INFO,
         PSTR("TCC: DMX TX UART%d rx=%d tx=%d (%d slots)"),
         uart, rx, tx, TC_DMX_SLOTS);
  return 1;
}

static void tc_dmx_ensure_init(void) {
  if (tc_dmx_active) return;
  if (TC_DMX_TX_PIN >= 0) {                     // compile-time fallback
    tc_dmx_init(TC_DMX_UART_NUM, TC_DMX_RX_PIN, TC_DMX_TX_PIN);
    return;
  }
  if (!tc_dmx_warned) {
    tc_dmx_warned = true;
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: dmxWrite ignored — call "
           "dmxInit(uart,rx,tx) first (or #define TC_DMX_TX_PIN)"));
  }
}
...
static void tc_dmx_send_frame(void) {
  if (!tc_dmx_active || !tc_dmx_uart || tc_dmx_cfg_uart < 0) return;
  tc_dmx_uart->flush();                         // ensure line idle (high)
  // BREAK: drive TX low by inverting the idle level.
  uart_set_line_inverse((uart_port_t)tc_dmx_cfg_uart, UART_SIGNAL_TXD_INV);
  delayMicroseconds(TC_DMX_BREAK_US);
  uart_set_line_inverse((uart_port_t)tc_dmx_cfg_uart, 0);  // MAB (high)
  delayMicroseconds(TC_DMX_MAB_US);
  tc_dmx_uart->write((uint8_t)0);                // start code 0x00
  tc_dmx_uart->write(tc_dmx_buf, sizeof(tc_dmx_buf));
}

// Called from FUNC_EVERY_50_MSECOND (~20 Hz). Refresh + watchdog.
...
#else
static inline int  tc_dmx_init(int, int, int) { return 0; }
...
static int tc_syscall(TcVM *vm, uint16_t id) {
  ...
  switch (id) {
    ...
    case SYS_DMX_WRITE:
      ...
      break;
    case SYS_DMX_INIT: {
      int dtx = TC_POP(vm); int drx = TC_POP(vm); int du = TC_POP(vm);
      TC_PUSH(vm, tc_dmx_init(du, drx, dtx));   // (uart,rx,tx) -> 1/0
      break;
    }
