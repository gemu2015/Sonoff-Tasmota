...
static void tc_vm_task(void *param) {
  ...
  //
  // Hold vm_mutex around each 256-instruction batch, mirroring the Phase-2
  // TaskLoop section. Without this, Tasmota's main-thread callback dispatch
  // paths (Command, WebCall, WebOn, BootInit, OnMqttData, FUNC_EVERY_SECOND
  // ticks, etc.) only protect themselves with the !vm.halted gate — they
  // don't actually serialise against Phase-1's tc_vm_step execution.
  // Andreas (.107 / Bat3) hit this as a ~30-50% Slot-0-restart Russian-
  // roulette crash with fingerprint `err=12 (Frame locals NULL) PC=N
  // idx=M fp=0 fc=0 instr=0`: a callback firing during the brief Phase-1
  // window between tc_vm_task starting and main() reaching the first
  // user-function OP_CALL corrupted vm->fp / frame_count, and the next
  // STORE_LOCAL then hit the defensive guard. Holding the mutex during
  // every step makes the per-callsite `!halted` check actually correct.
  while (!slot->task_stop && !vm->halted && vm->error == TC_OK) {
    // Handle delay as real RTOS blocking delay (feeds WDT, yields CPU).
    // Release the mutex around the delay so Tasmota's main loop + callbacks
    // can run while we wait. Re-acquire afterwards.
    if (vm->delayed) {
    ...
    // Acquire the slot's vm_mutex before stepping. Callbacks held off
    // until we release it (either at end of the 256-instruction batch
    // for the yield(), or via the delayed-path above).
    if (slot->vm_mutex) xSemaphoreTake(slot->vm_mutex, portMAX_DELAY);
    tc_current_slot = slot;  // restore after mutex acquire — callbacks may have changed it
