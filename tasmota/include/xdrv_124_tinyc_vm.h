...
// ESP32-only — ESP8266 LwIP doesn't export igmp_leavegroup() at the public
// raw-API level (the function exists internally but the header isn't
// visible from user code). Ottelo's build (`tasmota4m_ottelo_tc` ESP8266)
// failed at link with "'igmp_leavegroup' was not declared in this scope"
// — my earlier "ESP8266 LwIP also exports the raw IGMP API" note in the
// 1.6.1 changelog was wrong. udp(10) returns 0 (= not supported) on ESP8266.
#ifdef ESP32
#include <lwip/igmp.h>
#endif
    ...
    case SYS_UDP_FUNC: {
      ...
      switch (mode) {
        ...
        case 10: { // udp(10, mcast_ip_ref) → IGMP-Leave (netif-level, socket-independent)
          ...
          // ESP32-only — ESP8266 LwIP doesn't expose igmp_leavegroup() at the
          // public API level (Ottelo's tasmota4m_ottelo_tc build failed link
          // 2026-05-13). On ESP8266 we pop the arg and return 0 (= not
          // supported); the script can fall back to udp(0, port) which at
          // least closes the socket even if it doesn't fully leave the group.
          int32_t ip_ref = TC_POP(vm);
#ifdef ESP32
          char ip_str[32];
          tc_ref_to_cstr(vm, ip_ref, ip_str, sizeof(ip_str));
...
#else
          // ESP8266: igmp_leavegroup not available via public LwIP header.
          (void)ip_ref;
          TC_PUSH(vm, 0);
#endif
          break;
