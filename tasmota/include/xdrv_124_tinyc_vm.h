...
enum TcSyscall {
  ...
  SYS_FAST_MUX        = 427, // (flag, time, buf_ref, len) -> int — HW-timer GPIO multiplexer (ESP32-S3 only, USE_TINYC_FAST_MUX); -1 if not built
  // HTTP
...
/*********************************************************************************************\
 * VM: Fast pin multiplexer  —  ESP32-S3 only, opt-in via USE_TINYC_FAST_MUX
 *
 ...
 *
 * S3-only: those fast registers are a plain 32-bit word on the S3 (and the feature wants a
 * spare HW timer + a core to spare); it makes no sense on single-core / other targets, so it
 * is gated to S3 + the opt-in build flag. On any other target fastMux() simply returns -1.
 * Note: the original Scripter version used the Arduino-core-2.x timer API (timerAlarmWrite,
 * etc.); this port uses the core-3.x API (timerBegin(freq) / timerAlarm / timerAttachInterrupt).
 *
...
\*********************************************************************************************/
#if defined(USE_TINYC_FAST_MUX) && defined(CONFIG_IDF_TARGET_ESP32S3)
...
#endif // USE_TINYC_FAST_MUX && CONFIG_IDF_TARGET_ESP32S3
...
static int tc_syscall(TcVM *vm, uint16_t id) {
  ...
  switch (id) {
    ...
    case SYS_FAST_MUX: {           // fastMux(flag, time, buf, len) -> int (ESP32-S3 + USE_TINYC_FAST_MUX)
      int32_t len  = TC_POP(vm);
