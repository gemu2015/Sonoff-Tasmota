...
static int tc_syscall(TcVM *vm, uint16_t id) {
  ...
  switch (id) {
    ...
    case SYS_MQTT_PUBLISH_REF:
      TC_POP(vm);
      if (id == SYS_MQTT_PUBLISH_TO || id == SYS_MQTT_PUBLISH_REF) TC_POP(vm);
      if (id == SYS_MQTT_PUBLISH_REF) TC_POP(vm);
      TC_PUSH(vm, -1);
      break;
#endif  // USE_MQTT
