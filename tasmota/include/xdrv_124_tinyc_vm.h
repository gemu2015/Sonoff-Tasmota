...
// -4 PASV refused, -5 data connection failed, -6 STOR/APPE refused,
...
static int32_t tc_ftp_store(const char *hostport, const char *user, const char *pw,
  ...
  int v[6] = {0};
  const char *p = (code == 227) ? strchr(line, '(') : nullptr;
  if (!p || sscanf(p + 1, "%d,%d,%d,%d,%d,%d", &v[0], &v[1], &v[2], &v[3], &v[4], &v[5]) != 6) {
    tc_ftp_cmd(ctrl, "QUIT", nullptr, line, sizeof(line), 1000); ctrl.stop();
    AddLog(LOG_LEVEL_INFO, PSTR("TCC: ftp %s PASV refused %d %s"), host, code, line); return -4;
  }
  // A NATed or misconfigured server announces 0.0.0.0 or a foreign address;
  // the data port is then on the SAME host as the control connection.
  IPAddress dip(v[0], v[1], v[2], v[3]);
  if (v[0] == 0 || v[0] == 127) dip = ctrl.remoteIP();
  const int dport = v[4] * 256 + v[5];
  WiFiClient data;
#ifdef ESP32
