...
enum TcSyscall {
  ...
  SYS_DMX_WRITE             = 396, // (channel, value) -> void  set DMX512 slot
                                  //   (1..TC_DMX_SLOTS) to 0..255. TX-only on UART2 via
                                  //   the inverse-line BREAK method; frame auto-refreshed
                                  //   ~22 Hz, 30 s watchdog → all-zero. Lazy UART init on
                                  //   first call (pins TC_DMX_TX_PIN / optional DE).
...
// ---------------------------------------------------------------------------
// DMX512 TX engine (TX-only, ESP32). Single dedicated hardware UART at
// 250000 8N2. Per-frame BREAK via the inverse-line method
// (uart_set_line_inverse) — no library, no per-frame begin(), compiles
// against the pinned arduino-esp32. dmxWrite(ch,val) just stores a slot;
// tc_dmx_tick() (FUNC_EVERY_50_MSECOND, ~20 Hz) emits the frame and runs
// a 30 s watchdog → all-zero (heater safe-off). Lazy init on first write.
//   Override in user_config_override.h:
//     TC_DMX_TX_PIN (default -1 = disabled until set), TC_DMX_DE_PIN
//     (RS485 driver-enable, -1 = none/tied), TC_DMX_UART_NUM (2),
//     TC_DMX_SLOTS (16).
// ---------------------------------------------------------------------------
#ifdef ESP32
#ifndef TC_DMX_TX_PIN
#define TC_DMX_TX_PIN     -1
#endif
...
static void tc_dmx_ensure_init(void) {
  if (tc_dmx_uart || tc_dmx_active) return;
  if (TC_DMX_TX_PIN < 0) {
    if (!tc_dmx_warned) {
      tc_dmx_warned = true;
      AddLog(LOG_LEVEL_INFO, PSTR("TCC: dmxWrite ignored — set "
             "#define TC_DMX_TX_PIN <gpio> in user_config_override.h"));
    }
    return;
  }
  tc_dmx_uart = new HardwareSerial(TC_DMX_UART_NUM);
  // 250000 baud, 8 data, no parity, 2 stop; RX unused (-1).
  tc_dmx_uart->begin(250000, SERIAL_8N2, -1, TC_DMX_TX_PIN);
  if (TC_DMX_DE_PIN >= 0) {
  ...
  AddLog(LOG_LEVEL_INFO, PSTR("TCC: DMX TX on UART%d GPIO%d (%d slots)"),
         TC_DMX_UART_NUM, TC_DMX_TX_PIN, TC_DMX_SLOTS);
}
...
static void tc_dmx_send_frame(void) {
  if (!tc_dmx_active || !tc_dmx_uart) return;
  tc_dmx_uart->flush();                         // ensure line idle (high)
  // BREAK: drive TX low by inverting the idle level.
  uart_set_line_inverse((uart_port_t)TC_DMX_UART_NUM, UART_SIGNAL_TXD_INV);
  delayMicroseconds(TC_DMX_BREAK_US);
  uart_set_line_inverse((uart_port_t)TC_DMX_UART_NUM, 0);  // MAB (high)
