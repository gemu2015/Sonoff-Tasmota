...
| UDP (Scripter globalvars) | `udpSend(name, val)`, `udpRecv(name)`, `udpReady(name)`, `udpRecvArray(name, arr, max)`. ⚠️ "Some `global` values never arrive" → console **`TinyCUdp`** first: per name `rx`/`inj` + totals. Since 2026-09-23 the ESP32 receives over a plain lwIP socket — `NetworkUDP::parsePacket()` on core 3.3.8 delivered 4 of ~1400 datagrams on a P4. A slot on the SAME device does not receive its own broadcast (use `shareSetFloat`). |
