...
| Serial | `serialBegin(rx, tx, baud, cfg, buf)` returns slot 0..2; `serialRead(slot)`, `serialWrite(slot, str)`, `serialAvailable(slot)` (multi-port API; slot is first arg). ⭐ **`serialReadArray(slot, arr, n)` reads a whole BLOCK in one syscall** — `serialRead()` costs one syscall per byte, measured at 95,969/s on an ESP32-S3, i.e. a quarter of the VM just to fetch 230,400 baud. `arr` may be `int[]` or `byte[]`. ⚠️ `serialWriteBytes()` wrote NOTHING for `len > 256` until 2026-09-13 — silently; it now chunks. |
...
| FTP | One session per device: `ftpOpen(host, user, pw)` / `ftpClose()`; then `ftpPut(remote, local, mode)` (file), `ftpPutStr(remote, data, mode)` (char[]), `mode` 0 = replace, 1 = APPEND; `ftpGet(remote, local)`, `ftpGetStr(remote, buf)`, `ftpList(dir, buf)` (names, one per line, returns count), `ftpSize`, `ftpDelete`, `ftpMkdir`, `ftpRename(from, to)`. Returns bytes/0 or -1…-10 (-10 = no session). Blocking: call from `main()` or `TaskLoop()`, never from `EverySecond` (mutex held for the whole transfer) and not from a `spawnTask` worker when the data lives in a `char[]` (own VM, own heap). Fritzbox: FTP enabled under Heimnetz → Speicher, internal storage `/FRITZ/mediabox/…`, use the box's IP (name → IPv6 → PASV refused; EPSV fallback exists), the DLNA media server is read-only |
...
| UDP (Scripter globalvars) | `udpSend(name, val)`, `udpRecv(name)`, `udpReady(name)`, `udpRecvArray(name, arr, max)`. ⚠️ "Some `global` values never arrive" → console **`TinyCUdp`** first: per name `rx`/`inj` + totals. Since 2026-09-23 the ESP32 receives over a plain lwIP socket: on a P4 (core 3.3.8, Ethernet+WiFi) the NetworkUDP socket built at boot sat at 4 of ~1400 datagrams per 30 s until rebuilt; rebuilt fresh it managed ~80 % of the plain socket (`TinyCUdp 1`/`0` switches for A/B). Root cause of the boot state still open — slot count proved irrelevant. A slot on the SAME device does not receive its own broadcast (use `shareSetFloat`). |
...
| WebChart (Google) | `WebChart`, `WebChartSize`, `WebChartTimeBase`, `WebChartJS`. ⚠️ `ymin`/`ymax` are runtime floats and all-or-nothing (`ymin >= ymax` = auto-scale) — for "start at zero, top automatic" use `WebChartJS("o.vAxis.viewWindow={min:0}")` after the `WebChart()`. ⚠️ `WebChartSize` is ONE setting per page render, not per script: it carries from slot to slot in slot order, so a script that omits it inherits the previous slot's width instead of the default. Call it explicitly (width `0` = full container width) in every chart script. Since 1.6.54 the default is full width × 300 px (was a fixed 960 px, wider than a phone viewport). Do NOT wrap chart blocks in `margin-left:-30px` any more — that old centering trick was for fixed widths and now pushes the chart off the left edge. |
...
| USB-Host seriell — FTDI am OTG-Buchse (**S3/S2 only**, `USE_TINYC_USBSERIAL`) | `usbInit()` (1 = Host laeuft), `usbState()` (0=aus 1=bereit 2=FTDI erkannt 3=offen 4=Fehler), `usbOpen(baud)`, `usbAvailable()`, `usbRead(buf, max)`, `usbWrite(buf, n)`, `usbClose()`, `usbDeinit()` (Stapel ab, RAM zurueck), `usbInfo(sel)` (0=vid 1=pid 2=rx 3=tx 4=verlorene Bytes). Der ESP32 ist hier der HOST: ein Geraet mit fest eingebautem FTDI (VarioLab!), an dessen UART man sonst nicht herankommt, wird zur seriellen Schnittstelle. Nur FT232R/FT232RL (0403:6001) — FTDI ist herstellerspezifisch, nicht CDC. ⚠️ **Jedes IN-Paket des FTDI beginnt mit ZWEI Statusbytes** (Modem + Leitung), die der Treiber abschneidet — wer selbst am Treiber baut, vergisst das und bekommt alle 62 Byte zwei Muellbytes ins Protokoll. ⚠️ Der Host muss VBUS liefern; meldet sich nichts an, dort zuerst messen. ⚠️ In `TaskLoop()` pumpen, nicht in einem Callback — 23 kB/s klemmen sonst Tasmotas Hauptschleife. ⭐ **Durchsatz hängt an der USB-Ereignisrunde, nicht an der Leitung.** Die Rückrufe der Übertragungen werden in DERSELBEN Aufgabe abgefertigt wie die Bibliotheksereignisse; wer dort blockierend wartet, bekommt je Runde nur ab, was gerade fertig ist. Am 12.09.2026 gegen ein echtes VarioLab gemessen: mit `usb_host_lib_handle_events(50 ms)` genau 4 IN × 62 B je Runde = **4,96 kB/s** empfangen und 1 OUT × 64 B = **1,2 kB/s** gesendet — beides Rechenergebnisse der Runde, auf das Byte. Die Leitung trägt 23 kB/s. Also `usb_host_lib_handle_events(0, …)` und das Warten dem Klienten-Handler überlassen (An- und Abstecken sind Sekunden auseinander, Übertragungen Millisekunden). **Danach gemessen: 22.349 B/s gesendet = 97 % der Leitung, 262.144 von 262.144 Byte, 0 verloren**; empfangen 18,5 kB/s aus dem VarioLab (soviel schickt es mit der schwersten brauchbaren .def), zweimal 15 s mit 0 verloren. ⚠️ Dabei fiel das Nächste auf: mit 2048 Byte Ring gingen bei 17,7 kB/s **3 Byte** verloren — in diesem Protokoll ist das ein verschobener Satz, kein fehlender Wert. Der Ring ist jetzt 16 kB (0,7 s Vorrat statt 89 ms), das Skript liest 1024 statt 256 Byte je Durchgang. ⚠️ Die IN-Puffer müssen **64 Byte** bleiben: der FTDI legt seine zwei Statusbytes vor JEDES 64-Byte-Paket, ein grösserer Puffer sammelte mehrere und der Treiber schnitte nur die ersten zwei ab. Mehr Durchsatz kommt über mehr gleichzeitige Übertragungen (`TC_USB_IN_N`), nicht über grössere. Beispiel: `examples/usb_variolab_tcp.tc` (TCP-Bruecke, uVariotest spricht ueber Port 2000 mit dem Geraet). |
