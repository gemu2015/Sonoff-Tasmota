...
**FTP:** `ftpOpen`, `ftpClose`, `ftpPut`, `ftpPutStr`, `ftpGet`, `ftpGetStr`, `ftpList`, `ftpSize`, `ftpDelete`, `ftpMkdir`, `ftpRename` — files on an FTP server (FRITZ!NAS …): write or append, read, list, manage
...
**USB host serial (ESP32-S3/S2, `USE_TINYC_USBSERIAL`):** `usbInit`, `usbState`, `usbOpen(baud)`, `usbAvailable`, `usbRead`, `usbWrite`, `usbClose`, `usbDeinit`, `usbInfo` — the ESP32 as USB **host** for an FT232R adapter: reaches the UART of a device whose FTDI is soldered in. Pump in `TaskLoop()`. See `examples/usb_variolab_tcp.tc`
