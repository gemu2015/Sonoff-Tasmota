...
**USB-Host seriell (ESP32-S3/S2, `USE_TINYC_USBSERIAL`):** `usbInit`, `usbState`, `usbOpen(baud)`, `usbAvailable`, `usbRead`, `usbWrite`, `usbClose`, `usbDeinit`, `usbInfo` — der ESP32 als USB-**Host** fuer einen FT232R: erreicht die serielle Schnittstelle eines Geraets, dessen FTDI fest eingebaut ist. In `TaskLoop()` pumpen; gemessen 22,3 kB/s = 97 % der Leitung bei 230400 Bd. Siehe `examples/usb_variolab_tcp.tc`
