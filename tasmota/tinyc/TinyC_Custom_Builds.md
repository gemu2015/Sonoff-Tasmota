...
| `-DUSE_TINYC_REPO_IDE` | Serve `/tcrepo`: pick a repo example, a slot, hit Run — compiled in the **browser** with the compiler pulled from the repo, then uploaded and started. The device stores only the page (**+6.5 KB flash**, no RAM), never the IDE. Must be served by the device: an https page cannot reach a plain-http LAN device (mixed content), so hosting it on the docs site is not an option. Needs internet in the browser, not on the device | Opt-in |
| `-DUSE_TINYC_ESPDL` | Neural-network inference through **ESP-DL** — `TinyCDl`, `TinyCDlCam` and the syscalls `camControl(21/22)`. **ESP32-S3 only, +759 KB flash** (core +700 KB, `vision/detect` +59 KB). Models are loaded at runtime as FILES, so they never enter the firmware image. Needs `lib/libesp32_dl` on `lib_deps` — see `lib/libesp32_dl/README.md`, the include paths there are not optional | Opt-in |
| `-DUSE_TINYC_NO_WORKER_VM` | **Opt out of the dual-context worker VM** (ESP32). By default a `spawnTask()` worker runs on its **own** VM (aliasing the slot's `globals[]`), so a background worker **and** a local web UI / LCD / `EverySecond` coexist on one slot. This flag reverts to the legacy path — the worker borrows the slot's single VM and the slot goes **headless** while it lives — saving a small per-worker VM overhead. See TinyC_Reference.md "Concurrency model". | Default: **worker VM ON** (ESP32) |
...
- **HomeKit** — Apple Home integration via `-DTINYC_HOMEKIT`. **Only in the
  ESP32-S3 / 16 MB builds** by policy — it adds ≈152 KB; on 4 MB that would
  link but leaves too little headroom (see Flash Budget above), so the
  4 MB / C3 builds force-drop it.
...
| `-DUSE_TINYC_ESPDL` | `camControl(21)` person detection, `camControl(22)` read the result. Needs `USE_TINYC_CAMERA` for the frame source. ESP32-S3 only |
