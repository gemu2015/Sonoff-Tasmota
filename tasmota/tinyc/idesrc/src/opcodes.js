...
export const Op = {
    ...
    // Element i is byte i of the region — the same memory an int32 array uses,
    // read differently. See tinyc/docs/BYTE_ARRAYS.md.
    LOAD_LOCAL_BYTE:  0xA8,  // u8 base_idx; pop idx → push byte
    STORE_LOCAL_BYTE: 0xA9,  // u8 base_idx; pop val, pop idx → store byte
    LOAD_GLOBAL_BYTE: 0xAA,  // u16 base_idx; pop idx → push byte
    STORE_GLOBAL_BYTE:0xAB,  // u16 base_idx; pop val, pop idx → store byte
    LOAD_HEAP_BYTE:   0xAC,  // u8 handle; pop idx → push byte
    STORE_HEAP_BYTE:  0xAD,  // u8 handle; pop val, pop idx → store byte
    LOAD_REF_BYTE:    0xAE,  // u8 local_idx; pop idx → push byte via ref
    STORE_REF_BYTE:   0xAF,  // u8 local_idx; pop val, pop idx → store byte via ref

    // ── Packed 16-bit arrays ───────────────────────────────────────────────
    // Two bytes per element: (n+1)/2 slots, element i at ((int16*)base)[i].
    // Half the RAM of an int[] at full sensor resolution. Signedness sits in
    // the LOAD, so the stores are shared between int16 and uint16.
    // See tinyc/docs/INT16_ARRAYS.md.
    LOAD_LOCAL_I16:   0xB6,  // u8 base_idx; pop idx → push sign-extended
    STORE_LOCAL_I16:  0xB7,  // u8 base_idx; pop val, pop idx → store 16 bits
    LOAD_GLOBAL_I16:  0xB8,  // u16 base_idx
    STORE_GLOBAL_I16: 0xB9,  // u16 base_idx
    LOAD_HEAP_I16:    0xBA,  // u8 handle
    STORE_HEAP_I16:   0xBB,  // u8 handle
    LOAD_REF_I16:     0xBC,  // u8 local_idx
    STORE_REF_I16:    0xBD,  // u8 local_idx
    LOAD_LOCAL_U16:   0xBE,  // u8 base_idx; pop idx → push 0..65535
    LOAD_GLOBAL_U16:  0xBF,  // u16 base_idx
    LOAD_HEAP_U16:    0xC0,  // u8 handle
    LOAD_REF_U16:     0xC1,  // u8 local_idx
    ...
    // Fused opcode runs emitted by the peephole in codegen. They add NO
    // expressiveness: each must behave exactly like the sequence it replaces,
    // so the unfused form stays valid forever and a program means the same
    // thing either way.
    // ⚠️ APPEND-ONLY like the syscall table — a .tcb in the field encodes these
    // numbers. Never renumber, never redefine.
    LK32_OP_ST:     0xB3,   // (src u8, k i32, op u8, dst u8) wide-constant twin of LK_OP_ST
    LL_CMP_JZ:      0xB4,   // (a u8, b u8, cmp u8, target u16)  if !(La CMP Lb) goto target
    LK32_CMP_JZ:    0xB5,   // (a u8, k i32, cmp u8, target u16) if !(La CMP k)  goto target
                            //   the loop head — branch on FALSE, exactly like the JZ it replaces
    LK_OP_ST:       0xB1,   // (src u8, k i8, op u8, dst u8) locals[dst] = locals[src] OP k
    LL_OP_ST:       0xB2,   // (a u8, b u8, op u8, dst u8)   locals[dst] = locals[a] OP locals[b]
                            //   `op` is the ORDINARY opcode of the operation (ADD, MUL, ...), so ONE
                            //   superinstruction covers every operator instead of one per operator.
    INC_LOCAL:      0xB0,   // (idx u8, delta i8) locals[idx] += delta, no stack effect.
                            //   replaces LOAD_LOCAL·DUP·PUSH_I8·ADD·STORE_LOCAL·POP
    LLK_OP2_ST:     0xC2,   // (y u8, z u8, k i8, op_in u8, op_out u8, dst u8)
                            //   locals[dst] = locals[y] OP_OUT (locals[z] OP_IN k)
                            //   ⚠️ The ONE nested shape that actually occurs: across the 224
                            //   shipped examples it appears ten times, and EVERY time with an
                            //   i8 constant on the inside — never with three locals, never
                            //   with a wide constant (counted 2026-09-03). Hence exactly this
                            //   one opcode and no family, and no LLK32 twin: it would not have
                            //   fired in a single example.
                            //   Replaces LOAD_LOCAL·LOAD_LOCAL·PUSH_I8·<op>·<op>·STORE_LOCAL,
                            //   i.e. SIX opcodes in 10 bytes by one in 7.
...
export const Syscall = {
    ...
    WEB_CHART_Q:    545, // (scale, offset) -> void — affine decode for the NEXT WebChart: value = raw * scale + offset. ONE-SHOT. Lets a chart be fed from a packed byte[] instead of a float[].
    ...
    // (BlueRadios/Nordic-UART-style peripherals). The client above connects, does ONE
    // read/write/notify-wait, then disconnects — fine for a device that wakes, reports,
    // and sleeps, but wrong for a stream: the link drops before a second sample can ever
    // notify. UUIDs here are STRING literals (16-bit "180a" or full 128-bit) — a
    // proprietary UART-style service is essentially always 128-bit, which the one-shot
    // family's int16 svc/chr cannot reach at all.
    BLE_SPP_TARGET: 535, // (mac, addrtype, svc_uuid) -> int   bleSppTarget(): set persistent target
    BLE_SPP_CONNECT:536, // ()                        -> int   bleSppConnect(): BLOCKS -- TaskLoop only
    BLE_SPP_STATE:  537, // ()                        -> int   bleSppState(): 1=connected, 0=not
    BLE_SPP_SUB:    538, // (notify_uuid)             -> int   bleSppSub(): subscribe notify/indicate
    BLE_SPP_AVAIL:  539, // ()                        -> int   bleSppAvailable(): queued notify bytes
    BLE_SPP_READ:   540, // (buf, max)                -> int   bleSppRead(): drain the notify ring
    BLE_SPP_WRITE:  541, // (chr_uuid, buf, len)      -> int   bleSppWrite(): write WITHOUT disconnecting
    BLE_SPP_CLOSE:  542, // ()                        -> int   bleSppClose(): disconnect
    // One-shot: connect, list every service+characteristic (UUID + R/W/N/I) as text,
    // disconnect. Run this FIRST against an unknown device -- a proprietary UUID has no
    // datasheet lookup, the device has to be asked.
    BLE_GATT_DUMP:  543, // (mac, addrtype, out_buf)  -> int   bleGattDump(): text length
    ...
    TOUCH_GET:          492, // (sel)                -> int   touchGet(sel): firmware Touch_Status — 0=pressed,1=x,2=y, -1/-2=raw x/y
    LVGL_SET_FONT:      493, // (h,size)             -> void  lvglSetFont() — label font, snapped to montserrat_tasmota 10/14/20/28
    LVGL_IMAGE_SCALE:   494, // (h,sx,sy)            -> void  lvglImageScale() — per-axis image zoom, 256=100% (lv_image_set_scale_x/y)
    LVGL_LINE:          495, // (parent)             -> int   lvglLine() — lv_line handle (cheap radial/vector bar)
    LVGL_LINE_POINTS:   496, // (h,x1,y1,x2,y2)      -> void  lvglLinePoints() — set the 2 endpoints (absolute screen px)
    LVGL_LINE_STYLE:    497, // (h,rgb,width)        -> void  lvglLineStyle() — line colour + width (rounded)
    LVGL_LINE_POLY:        517, // (h,xs,ys,n)          -> void  lvglLinePoly() — one lv_line draws a whole N-point polyline
    LVGL_ARC_BG_ANGLES:    518, // (h,start,end)        -> void  lvglArcBgAngles() — arc sweep (135,45=270° dial; 180,360=semicircle)
    ...
    USB_INIT:              546, // ()                   -> int   Host anmelden
    USB_STATE:             547, // ()                   -> int   0 aus 1 bereit 2 FTDI da 3 offen 4 Fehler
    USB_OPEN:              548, // (baud)               -> int   1 = offen
    USB_AVAILABLE:         549, // ()                   -> int   wartende Bytes
    USB_READ:              550, // (buf,n)              -> int   usbRead() — nicht blockierend
    USB_WRITE:             551, // (buf,n)              -> int   gesendete Bytes
    USB_CLOSE:             552, // ()                   -> int   0
    USB_DEINIT:            553, // ()                   -> int   0
    USB_INFO:              554, // (sel)                -> int   0 vid 1 pid 2 rx 3 tx 4 verloren
    // ⚠️ Serial in BLOCKS. serialRead() returns ONE byte per syscall; measured
    // on an ESP32-S3 that is 95 969 calls per second, about 10 us per byte --
    // at 230400 baud a quarter of the VM would go into fetching alone. Takes
    // an int32 array or a packed byte[], like usbRead/tcpReadArray, so a
    // bridge needs no copying.
    // ⚠️ READ ONLY. Writing is serialWriteBytes(h, buf, len), which has been
    // there all along and takes char[], byte[] AND int32 arrays.
    SERIAL_READ_ARR:       555, // (h,arr,n)            -> int   bytes read
    // FTP client (V32): one session per device, mode 0=STOR 1=APPE
    FTP_OPEN:              556, // (host,user,pw)       -> int 0 ok / <0
    FTP_CLOSE:             557, // ()                   -> void
    FTP_PUT:               558, // (remote,local,mode)  -> int bytes or <0
    FTP_PUT_STR:           559, // (remote,data,mode)   -> int bytes or <0
    FTP_GET:               560, // (remote,local)       -> int bytes or <0
    FTP_GET_STR:           561, // (remote,buf)         -> int bytes in buf or <0
    FTP_LIST:              562, // (dir,buf)            -> int entries or <0
    FTP_SIZE:              563, // (remote)             -> int bytes or <0
    FTP_DELETE:            564, // (remote)             -> int 0 ok / <0
    FTP_MKDIR:             565, // (dir)                -> int 0 ok / <0
    FTP_RENAME:            566, // (from,to)            -> int 0 ok / <0
    SPP_WRITE:             530, // (buf,n)              -> int   sppWrite()
    SPP_CLOSE:             531, // ()                   -> int   sppClose()
    SPP_SCAN:              532, // (buf,n,sekunden)     -> int   sppScan() — Geraete suchen
    SPP_DEINIT:            533, // ()                   -> int   sppDeinit() — Stapel abbauen, Speicher zurueck
    LVGL_CHART_UPD:        534, // (chart_h, mode)      -> void  lvglChartUpdateMode() — 0=schieben, 1=umlaufend
    ...
    // Float blib call — fcall("name", a, b) -> float. Float args/return cross
    // as raw bits in integer regs (soft-float _32r ABI), so it also verifies
    // the bit-int float passing on the hard-float P4 (mirror of the PFB wrappers).
    BLIB_CALL_F:             371, // (name_const, float a, float b) -> float
    ...
    AUDIO_MICGAIN:  513, // (gain) -> void — set mic gain 1-100 via the audio plugin (Plugin_Query 42/11)
    LVGL_CANVAS:    514, // (parent) -> int handle — create an lv_canvas (an lv_image, so lvglImageScale/Angle/Pivot work on it)
    LVGL_CANVAS_IMG: 515, // (canvas_handle, img_slot) -> void — point the canvas at a PSRAM RGB565 image slot (zero-copy) + redraw (live cam frames)
    DSP_FREE_IMAGE: 516, // (img_slot) -> void — free one tc_img_store RGB565 slot so a per-frame dspLoadImageFromCam loop doesn't exhaust the 4

    // Minimal I2S output (standalone, no USE_I2S_AUDIO needed)
    I2S_BEGIN:      271, // (mclk, bclk, lrclk, dout, sampleRate) -> int — init I2S TX
    ...
    I2S_MIC_BEGIN:  498, // (mclk, bclk, lrclk, din, sampleRate) -> int — init I2S RX mic
    I2S_MIC_READ:   499, // (arr_ref, max) -> int — read int16 mono samples, returns count
    I2S_MIC_LEVEL:  500, // () -> int — RMS loudness 0..32767 over one block
    I2S_MIC_STOP:   501, // () -> void — stop the mic RX channel
    I2S_DUPLEX_BEGIN: 502, // (mclk, bclk, ws, dout, din, sampleRate) -> int — full-duplex TX+RX (WM8960 mic needs the I2S TX clock); then i2sWrite plays + i2sMicLevel reads at once
    // Raw TLS client (one connection at a time; ESP32; call from a TaskLoop — these BLOCK)
    TLS_CONNECT:     503, // (host_ref, port)      -> int  DNS+TLS connect (SNI); 0=ok, -1=fail
    TLS_WRITE:       504, // (str_ref)             -> int  write request string; bytes, -1 if not connected
    TLS_READ_LINE:   505, // (buf_ref)             -> int  read one line (to \n, strips \r); length, -1 none
    TLS_READ:        506, // (buf_ref, maxbytes)   -> int  read up to maxbytes raw; count
    TLS_AVAILABLE:   507, // ()                    -> int  bytes available
    TLS_CONNECTED:   508, // ()                    -> int  1 if connected
    TLS_STOP:        509, // ()                    -> void close + free
    BASE64_ENC:      510, // (in_ref, in_len, out_ref) -> int  base64-encode in_len bytes of in[] into out[] (binary-safe)
    UTC_SECS:        511, // ()                    -> int  current UTC unix epoch (true UTC, for stamps/signing)
    RSA_ENCRYPT:     512, // (n_ref, e_ref, pw_ref, out_ref) -> int  RSA PKCS#1 v1.5 type-2 encrypt with pubkey (n,e base64url), hex out; for RSA-encrypted-password logins
    ...
    MQTT_PUBLISH_REF: 544, // (topic_ref, payload_ref, level) -> int 0=ok or -1
                           // Laufzeit-Strings + Log-Stufe (0 = still)
...
export const SYSCALL_ABI = 32  // V32: + FTP client (556-566) -- ONE session per device: ftpOpen(host,user,pw)/ftpClose(); on it ftpPut(remote,local,mode) sends a FILE, ftpPutStr(remote,data,mode) a BUFFER, ftpGet/ftpGetStr fetch, ftpList lists, ftpSize/ftpDelete/ftpMkdir/ftpRename manage (FRITZ!NAS, any NAS); mode 0 = STOR (replace), 1 = APPE (append). A dropped idle link is re-opened silently on the next call. Rolf's question of 22.09.2026: the Fritzbox media server is DLNA and read-only, what the box offers for writing is SMB and FTP, and there is no usable SMB client for the ESP32 -- so FTP. A hand-written client on WiFiClient (Tasmota's USE_FTP is only a server), every step bounded, VM mutex released like httpPost. Returns bytes sent or -1..-9 (see the vm.h comment). Pure append, no .tcb format change. || PREVIOUS V31: + serialReadArray (555) -- read a whole BLOCK off a serial port in ONE syscall, into an int32 array or a packed byte[]. serialRead() has always cost one syscall PER BYTE; measured on a real ESP32-S3 (2026-09-13) the VM manages 95 969 serial syscalls per second against 333 333 for an empty loop -- about 10 us a byte, so 230400 baud (23 kB/s) would spend a quarter of the VM on fetching alone, before any work at all. Came out of the VarioLab bridge, which needed a second source next to the USB host: same device, but wired to plain RX/TX pins on any ESP32 instead of an FTDI on the OTG socket. Read-only on purpose: serialWriteBytes(h, buf, len) has been there all along and takes char[], byte[] AND int32 arrays. ⚠️ In the same change, serialWriteBytes stopped SILENTLY DROPPING everything when len > 256 -- no error, no return value, just nothing on the wire; it writes in chunks now. Pure append, no .tcb format change. || PREVIOUS V30: + FTDI am USB-Host (546-554) — usbInit/State/Open/Available/Read/Write/Close/Deinit/Info. Ein serieller Draht zu allem, was hinter einem FTDI am USB-Host haengt; das Protokoll lebt im SKRIPT, wie bei SPP. Entstanden, weil gemus VarioLab seinen FTDI eingeloetet hat und es KEINEN Weg an die UART davor gibt. ⭐ Der USB-Host-Stapel steckte bereits in Tasmotas arduino-esp32-Zweig (libusb.a fuer den S3); es musste NICHTS am Framework geaendert werden, und die esp-usb-Komponenten werden nicht gebraucht — ein FTDI ist herstellereigen, kein CDC. ⚠️ Jedes IN-Paket beginnt mit ZWEI STATUSBYTES, die abgeschnitten werden muessen. Reines Anhaengen. || PREVIOUS V29: NO new syscall -- ONE new OPCODE, LLK_OP2_ST (0xC2), the fusion for `x = y OP (z OP k)`. It closes the last gap of the fusion run: after the six fusions of 2026-08-08 the bench_int loop still spent 13 opcodes per iteration, and SIX of them were this one unfused line, `sum = sum - (i / 2)` -- load, load, push, operate, operate, store. It is EIGHT now: one instruction per source statement plus the loop head and the jump back, which is the floor for a stack VM. ⚠️ THE SHAPE WAS COUNTED, NOT GUESSED: across the 224 shipped examples it occurs ten times, and every time with an i8 constant on the inside -- never with three locals, never with a wide constant. Hence exactly one opcode and no family, and no LLK32 twin: it would not have fired in a single example. A pure fusion, NO new expressiveness -- the unfused form stays valid and a program means the same either way. ⭐ EQUIVALENCE CHECKED: 5600 cases (all 100 operator pairs x 8 constants x 7 start values, including INT_MIN, division by zero and shifts) fused against unfused in the JS VM -- ZERO differences, error messages included (scripts/check_fusion.mjs). ⚠️ As with V21, newer bytecode on older firmware dies with BAD_OPCODE mid-loop, so the loader refuses a .tcb whose abi_rev is higher than its own. The compiler stamps 29 exactly when it emits the opcode (hook in emit()). || PREVIOUS V28: NO new syscall -- a BUG FIX that changes what eleven existing ones accept, the same shape as V26 and found the same way. tc_ref_maxlen() has long answered in BYTES for a packed byte[], but webArg, webParse, jsonStr, fileReadDir, fileGetStr, smlRead, pluginQuery, tlsReadLine, tlsRead, pwlStr and sppScan still wrote one int32 SLOT per character: handing any of them a byte[] wrote four times the array's size straight over the neighbouring heap. Reported by Hans against webArg() into a `byte a[96]` -- the mail address came back as "o", first character right and string over, which is the harmless half of the same write. All eleven now go through tc_chr_put(). ⚠️ webParse was wrong in BOTH directions: it also READ its source as int32 slots, so a byte[] source yielded one character; it goes through tc_ref_to_cstr() now, which knows both packings. jsonStr had the same reading problem on its source argument. ⚠️ Nothing about such a call LOOKS different, so the compiler stamps abi_rev 28 whenever a byte[] reaches one of those arguments (BUILTINS[].byteAbi in codegen.js) -- an older device refuses the .tcb instead of corrupting its heap. A char[] argument is unaffected and keeps its old abi_rev. Measured on an ESP32-C3 with a byte[] and a char[] in one handler on one request: before byte[16]="o" / char[16]="otto@example.org", after both "otto@example.org", and UTF-8 survives. || PREVIOUS V27: KEIN neuer Syscall — zwoelf neue OPCODES (0xB6..0xC1) fuer gepackte 16-Bit-Arrays, `int16[]` und `uint16[]`. Zwei Bytes je Element statt vier: (n+1)/2 Slots, Element i bei ((int16*)basis)[i]. Der Zweck ist derselbe wie bei byte[], nur eine Stufe feiner — ein byte[] spart zwar das Vierfache, kostet aber Aufloesung, weshalb es ueberhaupt WebChartQ mit Skala und Versatz gibt; ein int16 traegt eine Temperatur in Hundertstel Kelvin, einen rohen ADC-Wert oder ein Modbus-Register OHNE Umrechnung, bei der Haelfte des Speichers eines int[]. Gemessen an einem Ring mit 1441 Werten: float 5,6 KB, int16 2,8 KB, byte 1,4 KB. ⚠️ Das Vorzeichen steckt im LADEN, nicht in der Ablage: STORE_*_I16 schreibt fuer int16 und uint16 dieselben sechzehn Bit, waehrend der Compiler aus dem deklarierten Typ die vorzeichenerweiternde (0xB6/0xB8/0xBA/0xBC) oder die nullerweiternde Ladung (0xBE..0xC1) waehlt. Deshalb kostet uint16 vier Opcodes statt acht. ⚠️ Das Kennbit in der Referenz wurde zum ZWEI-BIT-Feld erweitert (Heap 8-9, Global 16-17, Lokal 24-25): 00 = int32, 01 = byte, 10 = int16, 11 = uint16. Die 01 ist bitgleich das alte byte-Bit, jedes vorhandene .tcb bedeutet also unveraendert dasselbe — der ABI-Sprung haengt allein an den neuen Opcodes, und der Compiler stempelt ihn genau dann, wenn er einen davon ausgibt (Haken in emit()). Ein aelteres Geraet weist das .tcb ab, statt mitten in der Schleife an BAD_OPCODE zu sterben. || VORHER V26: KEIN neuer Syscall — eine FEHLERKORREKTUR, die aendert, was vorhandene akzeptieren. httpGet, httpPost, smlGetStr und tasmCmd fuellten ihren Zielpuffer mit int32-SLOTS, waehrend tc_ref_maxlen() fuer ein gepacktes byte[] laengst in BYTES antwortete: ein byte[] dorthin zu geben schrieb das Vierfache seiner Groesse und ueberschrieb die Nachbar-Arrays im Heap — lautlos, und erst ab einer Antwort, die weit genug reichte. Alle vier gehen jetzt ueber tc_chr_put(). ⚠️ Man sieht einem solchen Aufruf nichts an, deshalb stempelt der Compiler abi_rev 26, sobald ein byte[] an eines dieser Argumente geht (BUILTINS[].byteAbi) — ein aelteres Geraet weist das .tcb ab, statt seinen Heap zu zerlegen. Ein char[]-Argument bleibt unberuehrt. || VORHER V25: + WEB_CHART_Q (545) — WebChartQ(scale, offset), eine affine Umrechnung auf die Messwerte des NAECHSTEN WebChart. Der Zweck ist der gepackte byte[]: ein Byte je Messwert statt eines int32-Slots, plus Skala/Versatz, traegt eine Temperatur in 0,5-K-Schritten ueber -40..87,5 degC oder eine Feuchte in 0,4 % — bei einem VIERTEL des Speichers. Das ist die groesste einzelne Heap-Position auf einem C3: die mitgelieferten Beispiele halten drei float[1441]-Ringe zu je 5,7 KB, aus einem TC_MAX_HEAP von 64 KB, und der Heap wird beim Laden als EIN zusammenhaengender Block geholt. ⚠️ Der byte[]-Teil ist trotz des reinen Syscall-Anhangs KEIN reines Anhaengen: WebChart (166) behaelt Nummer und Signatur, aber der Compiler markiert ein byte[]-Arrayargument jetzt mit dem Packungs-Bit, und AELTERE Firmware streift das Bit ab und liest das Array als float-Slots — Unsinn, lautlos, ohne fehlenden Syscall, der darauf zeigen wuerde. Deshalb stempelt der Compiler abi_rev 25 auf jedes WebChart, dessen Arrayargument ein byte[] ist (_istBytesVar); ein altes Geraet weist das .tcb dann ab, statt Unfug zu zeichnen. Ein WebChart auf einem float[] erzeugt weiterhin byte-identisches Bytecode und behaelt sein altes abi_rev. Ebenfalls in dieser Fassung, und voellig ohne ABI, weil rein serverseitiges HTML: die Drahtform je Messwert wechselte von _tcA mit [x,y]-Paaren auf _tcAy mit blosser y-Liste plus x0/Schrittweite — die x-Werte waren immer die Folge -(count-1-i)*interval + timebase. Aus rund 14 Byte je Punkt wurden rund 6; eine 1441-Punkt-Reihe schrumpfte von ~20 KB Antwortkoerper auf ~8 KB. _tcA bleibt daneben stehen. || VORHER V24: + SYS_MQTT_PUBLISH_REF (544) — mqttPublish mit LAUFZEIT-Strings und Log-Stufe. Bisher nahm mqttPublish nur Literale, also war ein Thema, das den Geraetenamen traegt, gar nicht erreichbar: Hans ging deshalb ueber den Befehl `Publish`, also eine geparste Befehlszeile, um eine Funktion zwei Rahmen tiefer zu treffen. Und es log IMMER mit — zwei Zeilen alle fuenf Sekunden bei weblog 2; `MqttPublishPayload` kennt die Stufe LOG_LEVEL_NONE laengst, nur TinyC kam nicht heran. Reines Anhaengen: die Zwei-Literal-Form bleibt Syscall 297 und erzeugt byte-identisches Bytecode, die neue Form entsteht nur, wenn ein Argument keine Konstante ist oder eine dritte Zahl dasteht. || VORHER V23: + LK32_OP_ST (0xB3) fuer Konstanten ausserhalb i8, sowie der Schleifenkopf LL_CMP_JZ / LK32_CMP_JZ (0xB4/0xB5). Der Vergleich-und-Springe laeuft einmal je Runde in JEDER Schleife und ist damit die breiteste der Verschmelzungen. || VORHER V22: + LK_OP_ST/LL_OP_ST (0xB1/0xB2) -- `x = y OP z` bzw. `x = y OP konst` auf einfachen int-Locals in EINEM Opcode statt vier (laden, laden/schieben, rechnen, ablegen). Genau die Stelle, an der eine Stapel-VM gegen eine Register-VM verliert. || VORHER V21: + Superinstruktionen (0xB0+). Erste: INC_LOCAL (0xB0) fuer `i++` als ANWEISUNG -- ersetzt LOAD_LOCAL/DUP/PUSH_I8/ADD/STORE_LOCAL/POP, also sechs Opcodes durch einen. Reine Verschmelzung, KEINE neue Ausdruckskraft: die unverschmolzene Form bleibt gueltig, ein Programm bedeutet dasselbe. ⚠️ Erster ABI-Sprung, bei dem NEUERES Bytecode auf AELTERER Firmware nicht mehr bloss einen fehlenden Syscall meldet, sondern mit BAD_OPCODE mitten in der Schleife stirbt -- deshalb weist der Lader ab sofort ein .tcb mit hoeherem abi_rev AB, statt es zu warnen und trotzdem zu laden. || VORHER V20: + BLE "SPP" (535-543) — bleSppTarget/Connect/State/Sub/Available/Read/Write/Close + bleGattDump. Der bisherige GATT-Client (bleTarget..bleResult) verbindet, macht EINEN Lese-/Schreib-/Notify-Vorgang und trennt danach — richtig fuer ein Geraet, das aufwacht, einmal meldet und wieder schlaeft, falsch fuer einen Strom: ein BlueRadios/Nordic-UART-artiges Geraet wuerde die Verbindung verlieren, bevor ein zweiter Messwert je ankommen koennte. Deshalb eine ZWEITE, unabhaengige NimBLE-Verbindung mit eigenem Notify-Ringpuffer, komplett in der TinyC-eigenen Datei (xdrv_79_tinyc_ble_glue.ino) — der bestehende GATT-Client (Waage, MI32, EQ3) bleibt unberuehrt. UUIDs sind hier ZEICHENKETTEN (16-Bit "180a" oder volle 128-Bit) statt der bisherigen int16-Variante — ein proprietaerer UART-Dienst ist so gut wie immer 128-Bit, int16 erreicht ihn gar nicht. bleGattDump() ist der Einzelvorgang davor: verbinden, alle Dienste+Merkmale auflisten, trennen — noetig, weil eine proprietaere UUID kein Datenblatt-Nachschlagen erlaubt. ⭐ VERIFIZIERT an echter Hardware 2026-08-05 (.39, ESP32-S3) gegen ein BlueRadios-Dualmodul: verbinden, BRSP_TX abonnieren, Datenmodus setzen, "VS\r" schreiben — Antwort kam in vier Haeppchen, und die Verbindung stand DANACH weiter. Pure append, kein .tcb-Formatwechsel. || VORHER V19: + lvglChartUpdateMode (534) — legt lv_chart_set_update_mode offen. 0 = schieben (LVGL-Vorgabe, macht die GANZE Diagrammflaeche je Wert ungueltig), 1 = umlaufend (wandernder Schreibstrich, nur eine schmale Spalte). Faktor ~250 bei einer 760x300-Flaeche — entscheidet, ob eine schnelle Messkurve ueberhaupt darstellbar ist. Pure append. || VORHER V18: + sppDeinit (533) — baut den Bluetooth-Classic-Stapel ab und gibt seine ~85 KB Heap zurueck. Ohne das scheitert der naechste Slot-Neustart an der Speicheranforderung, und das meldet sich als "Stack overflow". Pure append. || VORHER V17: + Bluetooth Classic / SPP (524-532) — sppInit, sppConnect (525 Literal / 526 char[]), sppState, sppAvailable, sppRead, sppWrite, sppClose, sppScan. Serial link to ANY Classic device; the protocol lives in the SCRIPT. ORIGINAL ESP32 ONLY (BR/EDR) and needs a firmware built with USE_TINYC_SPP against a Bluedroid framework. Pure append, no .tcb format change. || VORHER V16: + webCard (521, per-slot main-page card-frame toggle; webCard(0) renders bare like pre-card). Pure append. V15: + lvglLinePoly (517, one lv_line draws a whole N-point polyline) / lvglArcBgAngles (518, arc sweep) / lvglArcStyle (519, arc part colour+width — zoned gauges) / lvglRotate (520, rotate any object, for vertical y-axis titles). Pure append. V14: + lvglCanvas (514) / lvglCanvasSetImgSlot (515) / dspFreeImage (516) — a PSRAM RGB565 image slot (e.g. a HW-decoded camera frame from dspLoadImageFromCam) becomes an lv_canvas (an lv_image, so lvglImageAngle/Scale rotate+size it); dspFreeImage frees a slot so a live cam loop doesn't exhaust the 4. Pure append. V13: + audioMicGain (513, set mic gain 1-100 via the audio plugin, mirror of audioVol). Pure append. V12: + rsaEncrypt (512, RSA PKCS#1 v1.5 type-2 encrypt via BearSSL br_rsa_public, for RSA-encrypted-password logins). Pure append. V11: + utcSecs (511, current UTC unix epoch for stamps/signing). Pure append. V10: + raw TLS client (503-509: tlsConnect/tlsWrite/tlsReadLine/tlsRead/tlsAvailable/tlsConnected/tlsStop) + base64Enc (510) — a TinyC app can speak raw HTTPS (OAuth redirect/cookie flows, request signing), hot-reloadable. Pure append. V9: + SYS_I2S_DUPLEX_BEGIN (502, i2sDuplexBegin — full-duplex TX+RX for combined codecs like the WM8960 whose ADC is clocked by the I2S TX). Pure append. V8: i2sBegin (271) gained a leading mclk arg (i2sBegin(mclk,bclk,lrclk,dout,rate)) — existing syscall's arg count changed, so a mandatory bump. V7: + SYS_I2S_MIC_BEGIN/READ/LEVEL/STOP (498-501, mic RX / loudness). Pure append. V6: + SYS_LVGL_LINE/LINE_POINTS/LINE_STYLE (495-497, radial/vector bars). Pure append. V5: + SYS_LVGL_IMAGE_SCALE (494, lvglImageScale(h,sx,sy)). Pure append. V4: + SYS_LVGL_SET_FONT (493, lvglSetFont(h,size)). Pure append (old bytecode unaffected); bumped so the IDE flags a lvglSetFont .tcb on pre-font firmware. V3: + SYS_TOUCH_GET (492, touchGet(sel) -> Touch_Status). V2: + SYS_BLIB_CALL_F (371, fcall float blib call).
