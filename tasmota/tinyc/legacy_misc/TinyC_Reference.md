...
| `void addLog("fmt", val, ...)` | **Variadic** — printf-style formatting (see below) |
...
| `void addLogLevel(int level, "fmt", val, ...)` | **Variadic** — printf-style formatting at specific level |
| `webSendJsonArray(float arr[], int count)` | Emit float array as JSON integer array in web response |

**Variadic addLog (printf-style, ideal for debugging):**
...
addLog("x=%d", x);                                  // [LOG] x=42
addLog("t=%.1f", t);                                // [LOG] t=25.5
addLog("x=%d t=%.1f hello %s", x, t, name);         // [LOG] x=42 t=25.5 hello world
addLogLevel(2, "boot done x=%d!", x);               // INFO-level, trailing literal text OK
```

- Format string **must be a string literal** (the compiler splits it at compile time into a chain of single-value `sprintf` syscalls writing into a shared 160-byte scratch buffer, then calls the underlying `addLog`/`addLogLevel` syscall once).
- Specifiers: `%d %i %u %x %X %c` (int), `%f %e %g` and width/precision e.g. `%.1f` (float), `%s` (char[] or string literal).
- Output is **truncated to ~128 chars on device** (Tasmota's `LOGSZ`). Keep messages compact.
- Cost: a 160-byte (160-slot) heap scratch buffer is allocated **once per program** that uses the variadic form. Programs that only use `addLog("literal")` or `addLog(charBuf)` pay zero heap.
- Safe inside any context except `EveryLoop` (same constraints as the existing `addLog`).
...
| `webRepoPulldown(var, "label", "json_url", "index_key", "/dest_path")` | Remote-JSON directory picker (Scripter `smlpd` compatible). Fetches `json_url` client-side, expects `{"index_key":[{"label":"...","filename":"..."}, ...]}`, downloads the selected file from `{base}/{filename}` (base = json_url with trailing filename stripped) and POSTs it to `/tc_api?cmd=writefile&path=<dest_path>`. Selected index is written back to `var`. Pass `""` as `dest_path` to disable the download (selection-only) |
| `int smlApplyPins("/path", rx, tx, smlf)` | Idempotent pin substitution in an SML descriptor file. Recognizes `%0?rxpin%`, `%0?txpin%`, `%0?smlf%` placeholders (leading `0` optional, same regex as the IDE's "Replace pins" button). On the first call, inserts a `; <original>` template comment line above each line that contains a placeholder, then replaces placeholders in the active copy. On subsequent calls, rebuilds the active line **from the template comment** — so the original markers are always preserved and re-edits work. Pass `-1` for any value to leave that placeholder untouched. Returns the number of substitutions performed (`0` = file already in sync, no write; `-1` = error) |
| `int smlScripterLoad("/path")` | Extracts `>F` (every 100 ms) and `>S` (every second) Tasmota Scripter sections from an SML descriptor and runs them via a tiny built-in interpreter — so descriptors that drive an IEC mode-A handshake (or any tick-based logic) keep working in `-DTINYC_NO_SCRIPTER` builds without hand-porting to TinyC. Subset: `lnv0..lnv9` integer locals · `=`/`+=`/`-=`/`*=`/`/=` · `+ - * / %` · `< <= > >= == !=` · `switch <lnv> case <int> ... ends` · `if <expr> ... endif` · `sml(meter, 0, baud)` (set baud) · `sml(meter, 1, "HEX..")` (send raw bytes) · `;` line-comments. Anything outside this subset fails to compile (load returns `-1`, descriptor is unaffected). Returns the number of sections successfully compiled (0..2). Tick hooks fire only after a successful load; loading a new file replaces the previous bytecode |
...
**Example — `webRepoPulldown` for remote descriptor selection (Scripter smlpd compatible):**

```c
persist int meter_sel;   // remembered across reboots

void WebUI() {
    webRepoPulldown(meter_sel, "SML Descriptor",
...
On render the select is populated by a client-side `fetch` of the JSON. Selecting an
entry (a) writes the new index back to `meter_sel` via the usual `seva()` AJAX,
then (b) fetches `{base_url}/{entry.filename}` from GitHub and POSTs the body to
`/tc_api?cmd=writefile&path=/sml_meter.def` — saving the descriptor to the local
filesystem without a device reboot.

The JSON must follow Scripter's convention:
```json
{ "smartmeter": [ { "label": "eBZ DD3 2r06", "filename": "eBZ/eBZ DD3 2r06.tas" }, ... ] }
```
The `index_key` argument selects which array property to read — use `"files"` or any
other key for your own JSON manifests. Pass `""` as the `dest_path` for a
selection-only dropdown (no download).

Limitations:
- Files must be plain descriptors. Scripter SML descriptors containing `>S` / `>F`
  sections will not compile to TinyC and must be loaded via the Scripter editor
  instead — only pure meter descriptors and similar text files are supported.
- The JSON host must allow CORS (`Access-Control-Allow-Origin: *`).
  `raw.githubusercontent.com` does — most static hosts do too.

**Example — `smlApplyPins` for runtime descriptor pin editing:**

Many SML descriptors in the wild contain placeholder markers `%0rxpin%` /
`%0txpin%` (and the meter-ID filter `%0smlf%`) which the IDE's "SML descriptor"
tab replaces before upload. For users who simply load a pre-compiled `.tcb`
without ever opening the IDE, the same substitution can be done at runtime by
the running script. Use a free-pin picker for the user-facing widget and let
`smlApplyPins` keep `/sml_meter.def` in sync:
...
void WebUI() {
    webRepoPulldown(meter_sel, "Descriptor", URL, "smartmeter", "/sml_meter.def");
...
int main() {
    // -1 means "leave the placeholder text untouched". Without this, fresh persist
    // vars (which default to 0 — a real GPIO!) would write GPIO 0 into the file.
    if (rx_pin == 0 && tx_pin == 0) { rx_pin = -1; tx_pin = -1; }
    return 0;
}
```

How idempotency works — a freshly downloaded descriptor with a placeholder line:
```
+1,3,s,0,9600,SML_METER,1,1,%0rxpin%,%0txpin%
```
After the first call with `rx=17, tx=18`:
```
; +1,3,s,0,9600,SML_METER,1,1,%0rxpin%,%0txpin%
+1,3,s,0,9600,SML_METER,1,1,17,18
```
After a later call with `rx=25, tx=26` — the active line is rebuilt **from the
template comment**, so the original markers are never lost:
```
; +1,3,s,0,9600,SML_METER,1,1,%0rxpin%,%0txpin%
+1,3,s,0,9600,SML_METER,1,1,25,26
```
Calling repeatedly with unchanged values is a no-op (the function compares
the would-be output to the current file content and skips the write).

**Example — `smlScripterLoad` for IEC mode-A meters:**

Some old electric meters (Iskra MT174, AS1350, …) need a 300-baud wake-up,
then ACK, then a switch to 9600 to send data. The descriptor that drives this
contains a small `>F` section that ticks a counter and times the bytes:
...
switch lnv0
case 3
sml(1 0 300)              ; set meter 1 to 300 baud
sml(1 1 "2F3F210D0A")     ; send "/?!\r\n" — request identification
case 20
sml(1 1 "063035300D0A")   ; ACK + select mode B at 9600
case 23
sml(1 0 9600)             ; switch to data rate
case 60
lnv0=0                    ; reset counter every ~6 seconds
ends

>M 1
+1,%0rxpin%,o,0,9600,AS1350,%0txpin%
1,1.8.0(@1,Total In,kWh,Total_in,2
#
```

If you have full Tasmota Scripter compiled in, this just works — the Scripter
engine runs `>F` for you. In a `-DTINYC_NO_SCRIPTER` build, ask the TinyC
mini-scripter to take over:

```c
int main() {
    smlScripterLoad("/sml_meter.def");   // compiles >F (and >S if present),
                                          // runs them on Every100ms / EverySecond
    return 0;
}
```

`addLog` will report something like:
```
TCC: mscr compiled >F 134 src→52 bc
TCC: smlScripterLoad('/sml_meter.def') = 1 (>F=1 >S=0)
```

Notes:
- `lnv0..lnv9` are volatile (zeroed on each load, not persisted across reboots)
- Keep each section under ~384 bytes of bytecode (≈ 30–50 statements). Larger
  scripts should be ported to TinyC native (`Every100ms()` / `EverySecond()`)
- If full Scripter is **also** loaded, both engines run the same `>F` block →
  double work / double SML sends. Only use this when Scripter is off
- Calling `smlScripterLoad` again replaces the previous bytecode atomically
