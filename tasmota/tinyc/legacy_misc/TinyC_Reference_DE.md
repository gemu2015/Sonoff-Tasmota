...
| `void addLog("fmt", val, ...)` | **Variadisch** — printf-Stil-Formatierung (siehe unten) |
| `void addLogLevel(int level, char buf[])` | Ins Tasmota-Log auf bestimmter Ebene schreiben (1=ERROR, 2=INFO, 3=DEBUG, 4=DEBUG_MORE) |
| `void addLogLevel(int level, "literal")` | Zeichenketten-Literal auf bestimmter Ebene schreiben |
| `void addLogLevel(int level, "fmt", val, ...)` | **Variadisch** — printf-Stil-Formatierung mit expliziter Ebene |
| `webSendJsonArray(float arr[], int count)` | Float-Array als JSON-Integer-Array ausgeben |

**Variadisches addLog (printf-Stil, ideal zum Debuggen):**
...
addLog("x=%d t=%.1f hallo %s", x, t, name);         // [LOG] x=42 t=25.5 hallo world
addLogLevel(2, "boot fertig x=%d!", x);             // INFO-Ebene, Klartext am Ende OK
```

- Format-Zeichenkette **muss ein Zeichenketten-Literal sein** (der Compiler zerlegt sie zur Compile-Zeit in eine Kette von einzelnen `sprintf`-Syscalls, die in einen gemeinsamen 160-Byte-Scratch-Puffer schreiben, und ruft dann genau einmal den `addLog`-/`addLogLevel`-Syscall auf).
- Spezifizierer: `%d %i %u %x %X %c` (int), `%f %e %g` mit Breite/Genauigkeit z. B. `%.1f` (float), `%s` (char[] oder Zeichenketten-Literal).
- Ausgabe wird **auf dem Geraet auf ~128 Zeichen begrenzt** (Tasmotas `LOGSZ`). Halten Sie Meldungen kompakt.
- Kosten: ein 160-Byte-Heap-Scratch-Puffer wird **einmal pro Programm** allokiert, das die variadische Form nutzt. Programme mit nur `addLog("literal")` oder `addLog(charBuf)` zahlen kein Heap.
...
| `webRepoPulldown(var, "label", "json_url", "index_key", "/dest_path")` | Auswahl aus Remote-JSON-Verzeichnis (Scripter `smlpd`-kompatibel). Holt `json_url` clientseitig, erwartet `{"index_key":[{"label":"...","filename":"..."}, ...]}`, laedt die ausgewaehlte Datei von `{base}/{filename}` und sendet sie per POST an `/tc_api?cmd=writefile&path=<dest_path>`. `""` als `dest_path` deaktiviert den Download (nur Auswahl) |
| `int smlApplyPins("/path", rx, tx, smlf)` | Idempotente Pin-Ersetzung in einer SML-Descriptor-Datei. Erkennt `%0?rxpin%`, `%0?txpin%`, `%0?smlf%` (fuehrende `0` optional, gleiche Regex wie der "Pins ersetzen"-Knopf in der IDE). Beim ersten Aufruf wird ueber jeder Zeile mit Platzhalter eine Vorlagen-Kommentarzeile `; <original>` eingefuegt und die aktive Kopie ersetzt. Folgeaufrufe rekonstruieren die aktive Zeile **aus dem Kommentar** — die Original-Marker bleiben so erhalten und Re-Edits funktionieren. `-1` laesst einen Platzhalter unveraendert. Rueckgabe: Anzahl Ersetzungen (`0` = Datei bereits aktuell, kein Schreibvorgang; `-1` = Fehler) |
| `int smlScripterLoad("/path")` | Extrahiert `>F` (alle 100 ms) und `>S` (jede Sekunde) Tasmota-Scripter-Abschnitte aus einem SML-Descriptor und fuehrt sie ueber einen winzigen eingebauten Interpreter aus — so funktionieren Descriptor-Dateien mit IEC-Mode-A-Handshake (oder anderer tickbasierter Logik) auch in `-DTINYC_NO_SCRIPTER`-Builds, ohne in TinyC-nativen Code portieren zu muessen. Untermenge: `lnv0..lnv9` Integer-Lokale · `=`/`+=`/`-=`/`*=`/`/=` · `+ - * / %` · `< <= > >= == !=` · `switch <lnv> case <int> ... ends` · `if <expr> ... endif` · `sml(meter, 0, baud)` (Baudrate setzen) · `sml(meter, 1, "HEX..")` (Rohbytes senden) · `;` Zeilenkommentare. Alles ausserhalb dieser Untermenge schlaegt beim Compilieren fehl (Rueckgabe `-1`, Descriptor unveraendert). Rueckgabe: Anzahl erfolgreich compilierter Abschnitte (0..2). Tick-Hooks feuern erst nach erfolgreichem Laden; ein erneuter Aufruf ersetzt den vorherigen Bytecode |
